...
#### keyvar.groups: string or vector of strings specifying grouping variables in plotdata for which to calculate the ranges of values when defining colors AND for use as the faceting variable in ggplot (e.g., one subplot generated in the plotting space for each unique value of string or per unique combinations of strings). (Re: defining colors: E.g., if you have measurements from 2 samples with different min-max ranges for the 2 variables being used to color code, you can get them on the same quantile-to-color scale by defining the column name with the sample id, thus yielding a legend applicable across facet groups). If a vector of column names, the data will be subdivided in the order of those column names for both quantile normalization (color assignments) and faceting.
