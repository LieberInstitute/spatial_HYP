...
# make a palette adding colors for addnl clusts
fullpal <- Polychrome::createPalette(N=nclus,seedcolors = c(xvmhpal,xarcpal),range=c(25,75))

names(fullpal) <- c(names(xvmhpal),names(xarcpal),unique(bkcl$bjm_annot)[!(unique(bkcl$bjm_annot) %in% c(names(xvmhpal),names(xarcpal)))])
