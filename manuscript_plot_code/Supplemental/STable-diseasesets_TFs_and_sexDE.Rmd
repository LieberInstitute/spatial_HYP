...
gtab[,xARC_FDRpt1clusts:=apply(.SD,MARGIN = 1,function(x){ifelse(xendea[FDR<0.1&gene_name==x,.N]>0,paste0(xendea[FDR<0.1&gene_name==x,cluster_annotated],collapse=", "),"NO")}),.SDcols = "gene_symbol"]

gtab[,xVMH_FDRpt1clusts:=apply(.SD,MARGIN = 1,function(x){ifelse(xendev[FDR<0.1&gene_name==x,.N]>0,paste0(xendev[FDR<0.1&gene_name==x,cluster_annotated],collapse=", "),"NO")}),.SDcols = "gene_symbol"]
```
...
# save one with only pertinent genes as well
gtab2 <- gtab[(vARC_sexDE_fdrsig=="YES"|vVMH_sexDE_fdrsig=="YES"|vARC_sexDE_nomsig=="YES"|vVMH_sexDE_nomsig=="YES"|xARC_FDRpt1clusts!="NO"|xVMH_FDRpt1clusts!="NO"|xpr_vVMH_M!=xpr_vVMH_F|xpr_vARC_M!=xpr_vARC_F)][!is.na(disorders)|!is.na(tested_tf)]


gtab3 <- gtab2[disorders %in% grep(disorders,pattern="BMI",value=T,invert=T)|is.na(disorders)]
