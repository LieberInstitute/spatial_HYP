---
title: "SF-proportions_of_celltypes_neighboring_VMHARCcluster_cells"
...
```{r}
vmhcelldists <- readRDS("xenium_HYP/processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/05b-VMHdomain_VMHcells_NandProp_pertype_within_100-500u_and_50u_radii.RDS")

arccelldists <- readRDS("xenium_HYP/processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/05b-ARCdomain_ARCcells_NandProp_pertype_within_100-500u_and_50u_radii.RDS")
...
for (i in c(1:length(rads))){
    ...
    arccelldists3[[i]][,label1:=gsub(label1,pattern="^ARC_(.*)",replacement=paste0("xARC.","\\1"))]
}
...
for (i in c(1:length(rads))){
    ...
    vmhcelldists3[[i]][,label1:=gsub(label1,pattern="^VMH_(.*)",replacement=paste0("xVMH.","\\1"))]
    vmhcelldists3[[i]][,label1:=gsub(label1,pattern="^xVMH.Lateral_border",replacement="xVMH.LateralBorder")]
}
names(vmhcelldists3) <- rads

## subset these to 50 and 100 unit radii for sup figs
vmhcelldists3 <- vmhcelldists3[c(1:2)]
arccelldists3 <- arccelldists3[c(1:2)]
rads <- names(vmhcelldists3)
...
lapply(c(1:length(rads)),FUN=function(i){
    ...
    pdf(paste0("manuscript_plots/Supplemental/SFigparts-local_celltype_props_forVMHARCclusts/supercluster_props_in_",
