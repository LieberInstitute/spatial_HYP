...
sampmeds <- tmpcd[,.(median_spotGenes=median(sum_gene),median_spotUMIs=median(sum_umi),median_spot_mitoPct=median(expr_chrM_ratio)),by="sample_id"]
...
then, make violin plots of all individual spot values for each metric, faceted by sample:
```{r}
spotvals <- tmpcd[,.(spotGenes=sum_gene,spotUMIs=sum_umi,spot_mitoPct=expr_chrM_ratio,sample_id)]
# sum_gene is an integer column which will screw with melting, so fix
spotvals[,spotGenes:=as.numeric(spotGenes)]
...
vmhvals <- tmpcd[Visium.Domain=="VMH",.(spotGenes=as.numeric(sum_gene),spotUMIs=sum_umi,spot_mitoPct=expr_chrM_ratio,sample_id)]
...
arcvals <- tmpcd[Visium.Domain=="ARC",.(spotGenes=as.numeric(sum_gene),spotUMIs=sum_umi,spot_mitoPct=expr_chrM_ratio,sample_id)]
...
```{r}
pdf("manuscript_plots/Supplemental/SFig-UMI_gene_mito_Visium.pdf",height=11,width=8.5)
do.call("grid.arrange",list(arrangeGrob(grobs=visqcplots,ncol=1,nrow=4,heights = c(2,3,3,3))))
dev.off()
```