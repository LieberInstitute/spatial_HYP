...
p <- ggplot(visdeplt[assay=="VMH"], aes(y = -log10(P.Value), x = logFC,col=assay))+
    geom_point(size = 0.15,stroke=0.35) +
    ggtitle("VMH (Visium)")+
    ...
    geom_text_repel(seed = 23942,aes(y = -log10(P.Value),
        ...
        size = 1.5,
        show.legend=FALSE,
        nudge_x=1.25*sign(visdeplt$logFC),
        #seed=10403, 
        max.iter=100000,
        max.overlaps=10000,
        max.time=3,
...
# save
pdf("manuscript_plots/Fig3/3Aalt-Visium VMH Sex DE Volcano.pdf",height=2,width=1.95)
p
dev.off()
```
...
## points to label
xendomdeplt <- xendomdeplt[gene_name %in% c("PTGDS","SHANK3","TAC1"),label:=gene_name]
...
# save
pdf("manuscript_plots/Fig3/3Balt-Xenium VMH domain, all cells considered together.pdf",height=2,width=1.95)
p
dev.off()
```
...
```{r}
visdeplt <- visde[assay=="ARC"][gene_name %in% c("SHANK3","DCC","CHRM5","KCNMB2","RXRG"),label:=gene_name]


p <- ggplot(visdeplt, aes(y = -log10(P.Value), x = logFC,col=assay,col=assay)) +
    geom_point(size = 0.15,stroke=0.35) +
    ggtitle("ARC (Visium)")+
...
# save
pdf("manuscript_plots/Fig3/3Calt-Visium ARC Sex DE Volcano.pdf",height=2,width=1.95)
p
dev.off()
```
...
## points to label
xendomdeplt <- xendomdeplt[gene_name %in% c("SHANK3","DCC","RXRG"),label:=gene_name]
...
pdf("manuscript_plots/Fig3/3Dalt-Xenium ARC domain, all cells considered together.pdf",height=2,width=1.95)
p
dev.off()
