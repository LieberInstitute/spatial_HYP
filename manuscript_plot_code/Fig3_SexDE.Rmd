...
p <- ggplot(visdeplt, aes(y = -log10(P.Value), x = logFC,col=assay)) +
    ...
    ylab("-log10 (P-value)")+
    labs(color="Visium\nDomain")+
    theme(axis.text=element_text(size=7),axis.title=element_text(size=7.5),legend.key.height = ggplot2::unit(0.075,"in"),legend.key.width=ggplot2::unit(0.04,"in"),legend.position.inside = c(0.13,0.13),legend.position = "inside",legend.text=element_text(size=5),legend.box=element_blank(),legend.background=element_blank(),legend.title = element_text(size=6,vjust=0.5,margin=margin(0,0,0.01,0,"in")))
...
names(vmhpal) <-unique(xenvmhdeplt$cluster_annotated)

p <- ggplot(xenvmhdeplt, aes(y = -log10(P.Value), x = logFC_MvF,col=cluster_annotated)) +
    geom_point(size = 0.15) +
    ...
    theme(axis.text=element_text(size=7),axis.title=element_text(size=7.5),legend.key.height = ggplot2::unit(0.075,"in"),legend.key.width=ggplot2::unit(0.04,"in"),legend.position.inside = c(0.21,0.13),legend.position = "inside",legend.text=element_text(size=5),legend.box=element_blank(),legend.background=element_blank(),legend.title = element_text(size=6,vjust=0.5,margin=margin(0,0,0.01,0,"in")))
...
names(arcpal) <-unique(xenarcdeplt$cluster_annotated)

p <- ggplot(xenarcdeplt, aes(y = -log10(P.Value), x = logFC_MvF,col=cluster_annotated)) +
    ...
    ylab("-log10 (P-value)")+
    labs(color="ARC Cell\nType")+
    theme(axis.text=element_text(size=7),axis.title=element_text(size=7.5),legend.key.height = ggplot2::unit(0.075,"in"),legend.key.width=ggplot2::unit(0.04,"in"),legend.position.inside = c(0.11,.85),legend.position = "inside",legend.text=element_text(size=5),legend.box=element_blank(),legend.background=element_blank(),legend.title = element_text(size=6,vjust=0.5,margin=margin(0,0,0.01,0,"in")))
...
### 
tac1F <- hyp2[,hyp2$sample_id=="V12D05-350_C1"]
...
pani <- hypx["TAC1",hypx$sample_id=="X99_8741C"]
# 
...
## panel H: TAC1 in VMH/ARC domains for male donor and panel I: for female
```{r}
...
p <- p+
    scale_fill_continuous("log\ncounts",low= "#FFFFFF",high="#000000",limits=c(0,7))+
...
pan j/k: rxrg in same

```{r}
panj <- hypx["RXRG",hypx$sample_id=="X86_reg1"]
pank <- hypx["RXRG",hypx$sample_id=="X99_8741C"]
...
other attached packages:
 [1] ggstance_0.3.7                 ggrepel_0.9.5                 
 [3] BiocParallel_1.38.0            parallelly_1.38.0             
 [5] sf_1.0-16                      Polychrome_1.5.1              
...
[23] ggrastr_1.0.2                  ggtext_0.1.2                  
[25] ggplot2_3.5.1                  colorout_1.3-0.2              
...
loaded via a namespace (and not attached):
 ...
 [37] irlba_2.3.5.1             terra_1.7-78              units_0.8-5              
 [40] dqrng_0.4.1               commonmark_1.9.1          DelayedMatrixStats_1.26.0
 [43] codetools_0.2-20          DropletUtils_1.24.0       DelayedArray_0.30.1      
 [46] DT_0.33                   scuttle_1.14.0            xml2_1.3.6               
 ...
 [85] Cairo_1.6-2               markdown_1.13             jpeg_0.1-10              
...
[154] tiff_0.1-12               Matrix_1.7-0              ExperimentHub_2.12.0     
[157] benchmarkme_1.0.8         sparseMatrixStats_1.16.0  bit64_4.0.5              
[160] Rhdf5lib_1.26.0           KEGGREST_1.44.1           statmod_1.5.0            
[163] shiny_1.8.1.1             AnnotationHub_3.12.0      gridtext_0.1.5           
[166] memoise_2.0.1             bslib_0.7.0               benchmarkmeData_1.0.4    
[169] bit_4.0.5                
> sessioninfo::session_info()
─ Session info ────────────────────────────────────────────────────────────────────
 setting  value
