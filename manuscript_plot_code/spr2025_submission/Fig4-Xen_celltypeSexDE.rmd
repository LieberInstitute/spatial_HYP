...
Panel C: Show a DEG per VMH cluster, shown for all VMH xenium clusters in a boxplot datawide (at the level of pseudobulked samples). VMH_1: MC4R, VMH_2 (and 3): TAC1, VMH_1: CYP26A1, and VMH_1: MYT1L.
```{r}
...
vmhe.m <- melt(vmhe[rn %in% c("CYP26A1","MC4R","TAC1","MYT1L")],id.vars=c("rn","celltype","clus"))
...
pdf("manuscript_plots/Fig4/4C-VMH_DEGs_boxplot.pdf",height=3,width=4.67)
...
## for panel D, we want to do the same for three ARC genes of interest. For ARC_4 we have the pick of the litter--we'll go with CDH13, which has known associations to weight. For ARC_3, RXRG is pretty high up there signif wise. And ARC_1 has signif CYP26A1 DE. Lastly, MYT1L.
```{r}
...
# melt for genes of interest
arce.m <- melt(arce[rn %in% c("CYP26A1","RXRG","MYT1L","CDH13")],id.vars=c("rn","celltype","clus"))
...
pdf("manuscript_plots/Fig4/4D-ARC_DEGs_boxplot.pdf",height=3,width=4.67)
