...
xbpanorm <- xpba[cluster_id=="X7"&elist.rn=="MOBP"]
xpbanorm <- melt(xbpanorm,id.vars=c("elist.rn","cluster_id"),variable.name="samp",value.name="MOBP")
...
ggplot(xpbs[["vmh"]],aes(y=MOBPnorm,x=assay,fill=assay))+
    geom_violin(position=position_dodge(width=0.9),linewidth = 0.1,key_glyph=draw_square)+
...
ggplot(xpbs[["arc"]],aes(y=MOBPnorm,x=assay,fill=assay))+
    geom_violin(position=position_dodge(width=0.9),linewidth = 0.1,key_glyph=draw_square)+
