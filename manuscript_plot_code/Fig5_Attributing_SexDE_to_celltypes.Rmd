...
xenarcdeplt[gene_name %in% c("MYT1L","TAC1","MC4R","CYP26A1","TMEM233","EGR1","ESR1","PGR")&FDR<0.2,label:=gene_name]
...
p <- ggplot(xenarcdeplt, aes(y = -log10(P.Value), x = logFC_MvF,col=subclus_annot)) +
    ...
    scale_x_continuous(limits = c(min(xenarcdeplt$logFC_MvF),max(xenarcdeplt$logFC_MvF)),
                       expand=c(0.175,0.175))+
    theme(axis.text=element_text(size=7),axis.title=element_text(size=7.5),legend.key.height = ggplot2::unit(0.075,"in"),legend.key.width=ggplot2::unit(0.04,"in"),legend.position.inside = c(0.805,0.119),legend.position = "inside",legend.text=element_markdown(size=5,margin=margin(0,0,0,0,"in")),legend.box=element_blank(),legend.background=element_blank(),legend.title = element_text(size=6,vjust=0.5,margin=margin(0,0,0.01,0,"in")),legend.box.background = element_blank(),legend.key=element_blank())

# rasterize
p <- rasterize(p,layers="Points",dpi=450)

# save
pdf("manuscript_plots/Fig5/5B-XeniumARCcelltypes_sexDE.pdf",height=2.5,width=2.3)
p
dev.off()
```
...
## fix pseudobulk data cell types to match palette
unique(vmhe$celltype)
vmhe[celltype=="CRHR2",celltype:="*CRHR2*"]
vmhe[celltype=="LAMP5",celltype:="*LAMP5*"]
vmhe[celltype=="xVMH-Glia-Mixed",celltype:="xVMH-Glia Mixed"]
vmhe[celltype=="Lateral-Border",celltype:="xVMH Lateral Border"]
...
setnames(vmhe.m,"celltype","xVMH Neuron Cluster")
...
ggplot(vmhe.m,aes(x=`xVMH Neuron Cluster`,y=value,color=`xVMH Neuron Cluster`,fill = Sex))+
...
# fix cell types to match palette
# unique(arce$celltype)
arce[celltype=="AGRP",celltype:="*AGRP*"]
arce[celltype=="TAC3-ESR1",celltype:="*TAC3*-*ESR1*"]
arce[celltype=="*POMC*-Oligo",celltype:="*POMC*-Oligo Mixed"]
arce[celltype=="SLC17A7-Glia-Mixed",celltype:="*SLC17A7*-Glia Mixed"]
...
setnames(arce.m,"celltype","xARC Neuron Cluster")

# make the gene names markdown'ed so they appear in italics
arce.m[,rn:=paste0("*",rn,"*")]

pdf("manuscript_plots/Fig5/5D-ARC_DEGs_boxplot.pdf",height=3,width=3.5)
ggplot(arce.m,aes(x=`xARC Neuron Cluster`,y=value,color=`xARC Neuron Cluster`,fill = Sex))+
    geom_boxplot(outliers = FALSE,linewidth = 0.25)+
    geom_point(size=0.35,position=position_jitterdodge(jitter.width = 0.15))+
    scale_fill_manual(values=c("#FFFFFF","#000000"))+
    scale_color_manual(values=arcpal)+
