...
# 
# ggplot(as.data.table(cbind(colData(hypx[,hypx$sample_id=="X99_8741C"]),spatialCoords(hypx[,hypx$sample_id=="X99_8741C"]))),aes(x=10*sdimx,y=10*sdimy,col=supercl))+
#    rasterize(geom_point(size=0.01),dpi=300,dev="cairo_png")+
#    guides(color="none")+
#    scale_color_manual(values=xenpal)
# 
# dev.off()
...
```{r}
pand <- hyp2[,hyp2$sample_id=="V12D05-350_C1"]
...
pdf("manuscript_plots/Fig1/1D-V12D05_350_C1_keydomains.pdf",height=2,width=2)
p+
...
panf <- hyp2["NR5A1",hyp2$sample_id=="V12D05-350_C1"]
...
dev.off()
# ok three conditionals should do it: ARC points that are at array_row<30, or at array_col>25 & array_row<41; for VMH, remove the 4 points at array_col<5 & array_col>19
tmpcoldata <- rbind(tmpcoldata[(Visium.Domain=="ARC"&!(array_row<30|(array_col>25&array_row<41)))],
                    tmpcoldata[Visium.Domain=="VMH"&!(array_col<5&array_row>19)])
...
ahull <- gift_wrapping(as.data.frame(tmpcoldata[Visium.Domain=="ARC",.(array_col,array_row)]))

# the ARC hull points still come out a little goofy because of some points in perfect line with each other along the perimeter meeting VMH. so find the point where the points would in order be jumping across the ARC diagonally (after col 15 row 31) and insert 29,41; 41,43 after that point. 

addpoints <- as.data.frame(cbind(c(29,41),c(41,43)))
colnames(addpoints) <- c("array_col","array_row")
insertrow <- which(ahull[,1]==15&ahull[,2]==31)
...
pdf("manuscript_plots/Fig1/1F-V12D05_350_C1_NR5A1.pdf",height=2,width=2)
p+
...
Panel  is Visium expression of a WM marker, KLK6. (Our options for those are somewhat limited on the xenium side).
```{r}
## for escheR, we put the assay value into the colData for it to use
...
```{r}
pani <- hypx[,hypx$sample_id=="X99_8741C"]
...
pdf("manuscript_plots/Fig1/1I-X99_8741C_Xendomains.pdf",height=2,width=2)
p+
    scale_fill_manual(values=pal,na.value = NA)+
...
### J K L are the same genes in the xenium counterpart, X99_8741C
```{r}
panj <- hypx["NR5A1",hypx$sample_id=="X99_8741C"]
# 
...
pdf("manuscript_plots/Fig1/1J-X99_8741C_NR5A1.pdf",height=2,width=2)
p+
    scale_fill_continuous("log\ncounts",low= "#FFFFFF",high="#000000")+
...
K: POMC in X99_8741C
```{r}
pank <- hypx["POMC",hypx$sample_id=="X99_8741C"]
colData(pank)$`log counts` <- as.numeric(logcounts(pank)["POMC",])

p <- make_escheR(pank,y_reverse=FALSE)
p <- p |> add_fill("log counts",size=0.125,point_size = 0.125)

pdf("manuscript_plots/Fig1/1K-X99_8741C_POMC.pdf",height=2,width=2)
p+
    scale_fill_continuous("log\ncounts",low= "#FFFFFF",high="#000000")+
    geom_path(data=dompoly,aes(x=xpol,y=ypol,group=dompol,col=dompol),linewidth = 0.25)+
    scale_color_manual(values=pal,na.value = NA)+
