---
title: "01-Plot each VMH cell type subclustering result"
...
vmhsub <- fread("processed-data/06_Banksy_VMH-ARC_subclustering/01_BanksyVMHsubclusts_kg15-AGFk30_lam0-.2-.5-.8-1_res.6-1-1.4-1.8-2.2.txt")
vmhsub <- vmhsub[,c(1,77:ncol(vmhsub)),with=F]
## get clustering results to plot (all names of vmhsub except rn, which is the cell id)
vmhsub <- cbind(vmhsub$rn,vmhsub[,lapply(.SD,FUN=function(x){paste0("X",x)}),.SDcols=c(2:ncol(vmhsub))])
setnames(vmhsub,1,"rn")

pltvmhs <- names(vmhsub)[c(2:ncol(vmhsub))]

## subset to main-typing-VMH-cells, append coords
hypx.plt <- hypx.plt[key %in% vmhsub$rn]
hypx.plt <- merge.data.table(hypx.plt,vmhsub,by.x="key",by.y="rn")

rm(vmhsub)
```

### plot each VMH cell type subclustering result
```{r}
bplapply(pltvmhs,BPPARAM = SnowParam(workers = 8,type = "SOCK",exportglobals = TRUE,exportvariables = TRUE),BPOPTIONS=bpoptions(exports = c("hypx.plt"),packages=c("ggplot2","ggrastr","gridExtra","data.table")),FUN=function(a){
    setDTthreads(1)
    ...
    png(paste0("plots/06-ARCVMH_BanksySubclustering/01-VMHsubclust_",a,".png"),width=2000,height=7000)
...
loaded via a namespace (and not attached):
 ...
 [67] snow_0.4-4                Rcpp_1.0.12              
 [69] glue_1.7.0                SparseArray_1.4.8        
 [71] here_1.0.1                xfun_0.45                
 [73] EBImage_4.46.0            dplyr_1.1.4              
 [75] HDF5Array_1.32.0          withr_3.0.0              
 [77] BiocManager_1.30.23       fastmap_1.2.0            
 [79] boot_1.3-30               rhdf5filters_1.16.0      
 [81] fansi_1.0.6               spData_2.3.1             
 [83] rsvd_1.0.5                digest_0.6.36            
 [85] R6_2.5.1                  mime_0.12                
 [87] colorspace_2.1-0          wk_0.9.2                 
 [89] sccore_1.0.5              jpeg_0.1-10              
 [91] RSQLite_2.3.7             R.methodsS3_1.8.2        
 [93] config_0.3.2              utf8_1.2.4               
 [95] tidyr_1.3.1               generics_0.1.3           
 [97] rtracklayer_1.64.0        class_7.3-22             
 [99] httr_1.4.7                htmlwidgets_1.6.4        
[101] S4Arrays_1.4.1            spdep_1.3-5              
[103] uwot_0.2.2                pkgconfig_2.0.3          
[105] gtable_0.3.5              blob_1.2.4               
[107] htmltools_0.5.8.1         dotCall64_1.1-1          
[109] fftwtools_0.9-11          scales_1.3.0             
[111] png_0.1-8                 attempt_0.3.1            
[113] knitr_1.48                rstudioapi_0.16.0        
[115] rjson_0.2.21              curl_5.2.1               
[117] proxy_0.4-27              cachem_1.1.0             
[119] rhdf5_2.48.0              BiocVersion_3.19.1       
