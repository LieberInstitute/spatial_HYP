...
use a legend extraction function (from https://statisticsglobe.com/add-common-legend-to-combined-ggplot2-plots-in-r/#example-2-add-shared-legend-to-ggplot2-plots-using-gridextra-package ) to make a legend that we'll yank out once and put on page one of each plot packet
```{r}
...
rm(extract_legend,leggenes,legdat,legplt,resegsamps,genepanel,origsamps,comparecounts)
gc(full=T)
```
...
lapply(singsampdat,FUN=function(d){
    ...
    ggsave(path="plots/02_compareCellSegmentations_1.5vs1.7",filename = paste0("01-",unique(d$sample),".pdf"),plot = plotsout,width = 8.5,height=7.5)
})
