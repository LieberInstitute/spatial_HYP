...
tmp <- merge.data.table(
   as.data.table(colData(hypb), keep.rownames = T),
   as.data.table(spatialCoords(hypb), keep.rownames = T),
   by = "rn"
)
...
## putative idents: 1 blah 2 blah 3 blah etc


## X1-3 X5 etc noninteresting
## X4 etc interesting
...
pal[, colr := c(Polychrome::dark.colors(21), Polychrome::light.colors(20))]
...
for (i in c(1:nrow(pal))) {
   ...
   for (j in c(1:length(samps))) {
      ...
      plts[[j]] <- ggplot(tmp[sample_id == samps[j] &
                                 label == curtype], aes(x = sdimx, y = sdimy, color = label)) +
   ...
   png(
      paste0(
         "plots/05_Banksy_M0lam0_res2_multisamp/01-byclust_allsamps/",
         curtype,
         ".png"
      ),
      width = 10000,
...
for (i in c(1:length(samps))) {
   ...
   plt <- ggplot(tmp[sample_id == samps[i]], aes(x = sdimx, y = sdimy, color =
                                                    label)) +
   ...
   png(
      paste0(
         "plots/05_Banksy_M0lam0_res2_multisamp/01-bysamps_allclust/",
         samps[i],
         ".png"
      ),
      width = 4000,
...
[141] compiler_4.4.1            crayon_1.5.3             
...
[157] Rhdf5lib_1.26.0           KEGGREST_1.44.1          
