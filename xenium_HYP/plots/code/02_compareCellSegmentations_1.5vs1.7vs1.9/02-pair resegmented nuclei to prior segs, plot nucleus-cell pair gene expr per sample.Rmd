...
```{r}
resegs <- list.files("processed-data/01_xeniumranger1.7-resegment",pattern="02",full.names = T)
resegsampname <- gsub(resegs,pattern="^.*resegment/02_(.*)_wnuclei$",replacement="\\1")
...
names(resegnuccentroids) <- gsub(resegsampname,pattern="^02_(.*e_reg.*)_.*$",replacement="\\1")
...
legdat <- singsampdat[[1]][gene %in% leggenes]
...
pdf("plots/02_compareCellSegmentations_1.5vs1.7vs1.9/02a-colorlegend_for_02b.pdf",width=8.5,height=7.5)
legplt
dev.off()

rm(leggenes,tmppal,legdat,legplt)
gc(full=T)
```
...
lapply(singsampdat,FUN=function(d){
    ...
    ggsave(path="plots/02_compareCellSegmentations_1.5vs1.7vs1.9",filename = paste0("02b-",unique(d$sample),".pdf"),plot = plotsout,width = 8.5,height=7.5)
})
