---
title: "02-Nomsig Sex DE heatmaps for cell types within ARC or VMH domains"
...
```{r}
indomde <- fread("processed-data/08_VMH-ARC cell type sex DE within domains/02-4celltypeARC-and-VMH-dualassignedDomains_celltypeSexDE.txt")
...
pltgene <-indomde[FDR_inARC<0.05|FDR_inVMH<0.05|(ID %in% hypgenes & (P.Value_inARC<0.01|P.Value_inVMH<0.01)),ID]

## get interesting cell types
keeptypes <- grep(unique(indomde.plt$bjm_annot),pattern="ARC|VMH|Tanycyte|Periventricular|Supraoptic|TRH",value=T)
...
```{r}
indomde.plt.arc <- dcast(indomde.plt[,.(ID,bjm_annot,logFC_MvF_inARC)],ID~bjm_annot,value.var="logFC_MvF_inARC")

indomde.plt.arc.pvals <- dcast(indomde.plt[,.(ID,bjm_annot,P.Value_inARC)],ID~bjm_annot,value.var="P.Value_inARC")

indomde.plt.arc.fdrs <- dcast(indomde.plt[,.(ID,bjm_annot,FDR_inARC)],ID~bjm_annot,value.var="FDR_inARC")
...
png("plots/08_VMH-ARC cell type sex DE within domains/02-ARC domain cell type sex DE heatmap FDR.1 p05.png",width=6,height=7,units = "in",res=144)
...
indomde.plt.vmh <- dcast(indomde.plt[,.(ID,bjm_annot,logFC_MvF_inVMH)],ID~bjm_annot,value.var="logFC_MvF_inVMH")

indomde.plt.vmh.pvals <- dcast(indomde.plt[,.(ID,bjm_annot,P.Value_inVMH)],ID~bjm_annot,value.var="P.Value_inVMH")

indomde.plt.vmh.fdrs <- dcast(indomde.plt[,.(ID,bjm_annot,FDR_inVMH)],ID~bjm_annot,value.var="FDR_inVMH")
...
png("plots/08_VMH-ARC cell type sex DE within domains/02-VMH domain cell type sex DE heatmap FDR.1 p05.png",width=6,height=7,units = "in",res=144)
draw(htmap,heatmap_legend_side="bottom")
dev.off()
```
