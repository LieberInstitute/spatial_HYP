---
title: "03b-Plot ARC4type-VMH with ARC0.5 VMH0.2 domains"
...
read in tables of VMH/ARC assignments using 0.25 VMH /0.625 ARC for smoothing step 2, with corrections for a couple samples with holes/outlier VMH blebs 
```{r}
vmharc4 <- fread("processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/03b-ARCVMHdomains_2stepsmooth_VMH-k10-0.2-VMH-k200-0.2_ARC-k50-0.1_ARC-k500-0.5.txt")
```

### plot VMH, ARC, everything else per sample
```{r}
vmharc4.list <- split(vmharc4,vmharc4$sample_id)

dompal <- c("#22dd00","#890090","#888888")
names(dompal) <- c("VMH","ARC","other")

plts <- lapply(vmharc4.list,FUN=function(s){
    ggplot(s,aes(x=xcoord,y=ycoord,color=dualVMHARC4))+
        geom_point(size=0.525)+
        scale_color_manual(values=dompal)
})

png("plots/07_Domains-subdomains_by_knnSmoothing_Banksytypes/03b-ARCVMHdomains_2stepsmooth_VMH-k10-0.2-VMH-k200-0.2_ARC-k50-0.1_ARC-k500-0.5.png",width=4000,height=4000)
