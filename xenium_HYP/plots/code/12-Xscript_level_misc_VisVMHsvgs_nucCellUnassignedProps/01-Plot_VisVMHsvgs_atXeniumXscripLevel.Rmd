...
### here, we only care to retrieve info on transcripts from the small set of genes of interest that were chosen based on possible spatial variability seen in visium: PEG10, MAN1A1, ACVR1C, LEPR, SST, FRMPD2, GDF10, BDNF, NECAB2, TMEM233, CYP19A1.
...
xens <- lapply(xenouts,FUN=function(x){
    ...
    xendat@molecules$detected$outs <- xendat@molecules$detected$outs[c("PEG10","MAN1A1","ACVR1C","LEPR","SST","FRMPD2","GDF10","BDNF","NECAB2","TMEM233","CYP19A1")]
...
lapply(gns,FUN=function(g){
    plts <- mapply(x=xens,d=dombounds,n=names(xens),SIMPLIFY=FALSE,FUN=function(x,d,n){
    ## make the plot, scaling transcript point sizes to # transcripts detected
    ## point size of 0.1 with PEG10 (prob most abundant gene of these) so use something in that range to adjust other values to
    ggplot(x[[g]],aes(x=x_location,y=y_location))+
            rasterize(geom_point(size=(.05*nrow(x[["PEG10"]]))/nrow(x[[g]]),stroke=0),dpi=450,dev = "cairo_png")+
