...
### Here, we will use the more constrained ARC definition that only considered cell types 15/19/31/33 in the smoothing, and VMH as defined in this scheme. (Since we deconflict dual-labeled cells based on comparative KNN scores, the VMH as defined here vs. as defined using additional cell types to define ARC does not necessarily yield identical domains).
...
# also make an object that's just the oligodendrocytes collapsed into one big oligo pool for unrelated analyses
bkcl[bjm_annot %in% paste0("Oligodendrocyte_",c(1,2,3)),bjm_annot:="Oligo"]
hyp.oligo <- hypb[,colnames(hypb) %in% unlist(bkcl[bjm_annot=="Oligo",rn])]
hyp.oligo$banksy_clus <- "Oligo"
...
### clean up, then iterate thru DE analysis for each pseudobulked cluster
rm(agc,agcd,agsc)
...
fulldgelres.arc <- lapply(fulldgelres,function(x){as.data.table(x["de"],keep.rownames=T)})
fulldgelres.arc <- rbindlist(fulldgelres.arc,idcol="cluster")
setnames(fulldgelres.arc,"de.rn","gene_name")

### save elists in their own object for later plot use
elists.arc <- lapply(fulldgelres,function(x){as.data.table(x["elist"],keep.rownames=T)})
saveRDS(elists.arc,"processed-data/08_VMH-ARC cell type sex DE within domains/02c-pseudobulkARC_voomLmFitELists.RDS")

rm(elists.arc)
## fix leading "de." in colnames; append cell type annotations
setnames(fulldgelres.arc,gsub(names(fulldgelres.arc),pattern="^de\\.(.*)$",replacement="\\1"))
...
### clean up
rm(ag,clus,dgels,fulldgelres,hyp.fineadom,hyp.adom.ag)
gc(full=T)
```
...
### concatenate results for all cell types
fulldgelres.vmh <- lapply(fulldgelres,function(x){as.data.table(x["de"],keep.rownames=T)})
fulldgelres.vmh <- rbindlist(fulldgelres.vmh,idcol="cluster")
setnames(fulldgelres.vmh,"de.rn","gene_name")

### save the elists in their own object for later plotting
elists.vmh <- lapply(fulldgelres,function(x){as.data.table(x["elist"],keep.rownames=T)})
saveRDS(elists.vmh,"processed-data/08_VMH-ARC cell type sex DE within domains/02d-pseudobulkVMH_voomLmFitELists.RDS")

rm(elists.vmh)
## clean up leading "de." in colnames; append cell type annotations
setnames(fulldgelres.vmh,gsub(names(fulldgelres.vmh),pattern="^de\\.(.*)$",replacement="\\1"))
...
## save
fwrite(fulldgelres.arc,"processed-data/08_VMH-ARC cell type sex DE within domains/02a-4ARCtypesmoothed_withinARC_celltypeSexDE.txt")
fwrite(fulldgelres.vmh,"processed-data/08_VMH-ARC cell type sex DE within domains/02b-4ARCtypesmoothed_withinVMH_celltypeSexDE.txt")

```

## 04/23/25 repeat just the pseudobulking/filtering for oligos
```{r}
### repeat as above for VMH
hyp.olig.ag <- dreamlet::aggregateToPseudoBulk(hyp.oligo,assay="counts",sample_id="sample_id",cluster_id = "banksy_clus",BPPARAM = MulticoreParam(8))

# extract colData to feed into DGEList as the "samples" data
agcd <- as.data.table(colData(hyp.olig.ag),keep.rownames=T)
agcd[Date_Experiment_Began=="2/21/24",batch:="second"]
agcd[Date_Experiment_Began=="10/17/23",batch:="first"]
agcd <- as.data.frame(agcd)
rownames(agcd) <- agcd$rn
...
fulldgelres <- lapply(dgels,FUN=function(X){
...
# save
saveRDS(fulldgelres[[1]],"processed-data/08_VMH-ARC cell type sex DE within domains/02z_AllOligosPbulked_for_retinoidgenecompare.RDS")
