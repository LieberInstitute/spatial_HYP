...
### use dreamlet to run random-donor-effects sex DE (mirroring the procedures used for visium) for the 4-cluster ARC definition. xenium run as covar.
```{r}
...
dl.rf <- dreamlet::processAssays(hyp.fineadom.ag,min.cells = 10,min.count = 10,min.samples = 5,min.prop = 0.4,normalize.method = "TMM",useCountsWeights = T,BPPARAM = MulticoreParam(8),formula = ~Sex+Date_Slides_Dropped_Off+(1|BrNum))

### Warning message: In dreamlet::processAssays(hyp.adom.ag, min.cells = 10, min.count = 10,  : Not enough samples retained or model fit fails: X21, X28, X29, X30, X34, X35, X36, X37, X40
...
dl.rf.res[,domain:="ARC_4type_hiconf"]
...
dl.rf <- dreamlet::processAssays(hyp.finevdom.ag,min.cells = 10,min.count = 10,min.samples = 6,min.prop = 0.4,normalize.method = "TMM",useCountsWeights = T,BPPARAM = MulticoreParam(8),formula = ~Sex+Date_Slides_Dropped_Off+(1|BrNum))
...
## save
fwrite(sexDE_indom,"processed-data/08_VMH-ARC cell type sex DE within domains/05-4celltypeARC-and-VMH-hiconf-interior96pctOfDomain_celltypeSexDE.txt")
```
