---
title: "03-All-cells VMH and ARC Domains smoothed with 5 ARC clusters-sex DE"
...
### we will use Yi's previous parameter sets for VMH/ARC smoothed domain definitions and subset to cells within those domains for sex DE analysis.
...
hypb$dualdom <- smoovedom$dualassignedSmooth2

hypav <- hypb[,hypb$dualdom %in% c("VMH_yi_2stage","ARC_yi_2stage")]
...
### use dreamlet to pseudobulk, then edgeR's voomLmFit to run sex DE (mirroring the procedures used for visium) for the bulk domains.
```{r}
hyp.adom.ag <- dreamlet::aggregateToPseudoBulk(hypav,assay="counts",sample_id="sample_id",cluster_id = "dualdom",BPPARAM = MulticoreParam(8))
...
fulldgelres <- rbindlist(fulldgelres,idcol="domain")
setnames(fulldgelres,"rn","gene_name")

## append cell type annotations
fulldgelres <- fulldgelres[,.(domain,gene_name,logFC,adj.P.Val,t,B,P.Value,AveExpr)]

### clean up
rm(ag,agc,agcd,agsc,clus,dgels,hypav,hyp.adom.ag)
gc(full=T)
```

## save
```{r}
fwrite(fulldgelres,"processed-data/08_VMH-ARC cell type sex DE within domains/03-5typeARC-and-dualassignVMH_domainwise_sexDE.txt",sep='\t',quote=F,row.names=F,col.names=T)

```

### notably here, only two genes (RDH10 ARC / CYP26A1 VMH) passses FDR significance at 0.05.
