---
title: "01-Cell Type DE within VMH Domain"
...
### use dreamlet to run random-donor-effects sex DE (mirroring the procedures used for visium). include slide as covar
```{r}
...
## min samples 5 -- size of the smaller sex group in total samps (females, one of whom lacks ARC); set min.prop to 0.1 since this is now mixed cell types in reality (vs min prop 0.4 for cell type-wise sex DE in scripts 08/01 and 02)
dl.rf <- dreamlet::processAssays(hyp.domwise.ag,min.cells = 10,min.count = 10,min.samples = 5,min.prop = 0.1,normalize.method = "TMM",useCountsWeights = T,BPPARAM = MulticoreParam(8),formula = ~Sex+Date_Slides_Dropped_Off+(1|BrNum))
...
## append rowData to result table

rddt <- as.data.table(rowData(hypav))
...
fwrite(dl.rf.res,"processed-data/08_VMH-ARC cell type sex DE within domains/03-5typeARC-and-dualassignVMH_domainwise_sexDE.txt",sep='\t',quote=F,row.names=F,col.names=T)

```

### notably here, nothing passses FDR significance at 0.05.
