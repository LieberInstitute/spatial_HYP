...
### use dreamlet to run random-donor-effects sex DE (mirroring the procedures used for visium) for ARC
```{r}
...
dl.rf <- dreamlet::processAssays(hyp.adom.ag,min.cells = 10,min.count = 10,min.samples = 6,min.prop = 0.4,normalize.method = "TMM",useCountsWeights = T,BPPARAM = MulticoreParam(8),formula = ~Sex+(1|BrNum))

### Warning message: In dreamlet::processAssays(hyp.adom.ag, min.cells = 10, min.count = 10, : Not enough samples retained or model fit fails: X10, X17, X21, X28, X31, X32, X34, X36, X37, X40)
...
### use dreamlet to run random-donor-effects sex DE (mirroring the procedures used for visium) for VMH
```{r}
...
## min samples 6 -- size of the smaller sex group in total samps (females)
dl.rf <- dreamlet::processAssays(hyp.vdom.ag,min.cells = 10,min.count = 10,min.samples = 6,min.prop = 0.4,normalize.method = "TMM",useCountsWeights = T,BPPARAM = MulticoreParam(8),formula = ~Sex+(1|BrNum))
