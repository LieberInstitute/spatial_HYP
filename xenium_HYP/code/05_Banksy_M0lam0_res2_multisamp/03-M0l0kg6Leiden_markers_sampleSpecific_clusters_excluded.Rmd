...
run registration wrapper. use min cells = 50 since we have much lower per-cell gene content (often 10s to low 100s of transcripts) than would be in seq-based measaures
```{r}
...
droplv <- tmpcd[,.N,by="sampclus"][N<50,sampclus]
...
hypb.retainedclusts.mk <- spatialLIBD::registration_wrapper(hypb,var_registration = "label",var_sample_id = "sample_id",gene_ensembl = "ID",gene_name = "Symbol",min_ncells = 50)
