---
title: "01-VMH_domain_cellclustering_Banksy"
...
## load SFE and annot clusters; subset to VMH spatial domain we defined by smoothing, regardless of cell type
...
smoove <- fread("processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/03b")

smoove <- DataFrame(smoove,row.names=smoove$rn)
smoove <- smoove[colnames(hypb),]
colLabels(hypb) <- smoove$dualVMHARC4

hypvmh <- hypb[,hypb$label=="VMH"]
```
...
# vmhclus <- bplapply(parm,BPPARAM=MulticoreParam(4),function(x){
vmhclus <- mclapply(parm,mc.cores=3,function(x){
  clust <- as.data.table(colData(
      clusterBanksy(hypvmh,
                         assay_name=useassay,
...
```{r}
i<-1
for (i in c(1:length(vmhclus))){
    if(i==1){vmhclus2 <- copy(vmhclus[[1]])}
    else{
        vmhclus2 <- merge.data.table(vmhclus2,vmhclus[[i]],by="rn")
    }
}

fwrite(vmhclus2,"processed-data/09_Banksy_clustering_in_VMH-ARC_doms/01-BanksyVMHdomainOnly_clustering_kg15-AGFk30_lam0-.2-.5-.8-1_res.6-1-1.4-1.8-2.2.txt")
```
