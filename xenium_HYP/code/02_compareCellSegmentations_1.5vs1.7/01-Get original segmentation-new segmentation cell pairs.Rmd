...
lapply(singsampdat,FUN=function(d){
    genes <- unique(d$gene)
    plts <- bplapply(genes,BPPARAM=bpp,BPOPTIONS = bpoptions(exportglobals =TRUE,packages=c("data.table","ggplot2","gridExtra")),FUN=function(g){
        ggplot(d[gene==g],aes(x=orig_value,y=reseg_value))+
            geom_point()+
    ...
    ggsave(path="plots/02_compareCellSegmentations_1.5vs1.7",filename = paste0(unique(d$sample),".pdf"),plot = plotsout,width = 7,height=7.5)
})
