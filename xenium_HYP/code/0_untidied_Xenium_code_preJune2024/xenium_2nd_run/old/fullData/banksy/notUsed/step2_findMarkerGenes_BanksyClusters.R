...
for(ksample in 6){
  ...
  Idents(se_seurat) = colData(se)[,"clust_M1_lam1_k50_res1.2"]
...
pdf("markerGenes_top10_banksy_lambda_1.pdf", width = 20, height = 20)
for(ksample in 1:6){
  print(ksample)
  ...
  # obj@data 
  obj = ScaleData(obj)
  Idents(obj) = colData(se)[,"clust_M1_lam1_k50_res1.2"]
    
  ######## 
  se_seurat_markers = readRDS(paste0(dir_data_processed_banksy, "se_seurat_sample",sampleID_tmp,"_markerGenesBanksy.rds"))
