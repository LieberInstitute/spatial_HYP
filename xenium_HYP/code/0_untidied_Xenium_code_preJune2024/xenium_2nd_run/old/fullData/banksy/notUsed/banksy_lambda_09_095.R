...
for(ksample in 6){
  ...
  #?? what value of k_geom did I use in the end?
  ...
  saveRDS(se, file = paste0(dir_data_processed_banksy, "se_sample",sampleID_tmp,"_preprosssed_lambda_09_095.rds"))
...
pdf("banksy_lambda_09_095.pdf", width = 40, height = 20)
for(ksample in 1:6){
  print(ksample)
  sampleID_tmp = sampleID_unique[ksample]
  se = readRDS(paste0(dir_data_processed_banksy, "se_sample",sampleID_tmp,"_preprosssed_lambda_09_095.rds"))
  cnames <- colnames(colData(se))
  cnames <- cnames[grep("^clust", cnames)]
  colData(se) <- cbind(colData(se), spatialCoords(se))
