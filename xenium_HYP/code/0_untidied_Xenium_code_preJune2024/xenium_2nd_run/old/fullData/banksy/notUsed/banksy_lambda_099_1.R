...
for(ksample in 6){
  ...
  saveRDS(se, file = paste0(dir_data_processed_banksy, "se_sample",sampleID_tmp,"_preprosssed_lambda_099_1.rds"))
...
pdf("banksy_lambda_099_1.pdf", width = 40, height = 20)
for(ksample in 1:6){
  print(ksample)
  sampleID_tmp = sampleID_unique[ksample]
  ...
  umap_lambdaV1 <- plotReducedDim(se,
                             dimred = grep("UMAP.*lam0.99$", rdnames, value = TRUE),
                             colour_by = cnames[1]
