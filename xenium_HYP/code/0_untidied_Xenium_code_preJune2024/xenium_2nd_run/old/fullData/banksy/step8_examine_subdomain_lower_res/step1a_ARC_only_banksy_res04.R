...
saveRDS(spe_joint, file = paste0("spe_joint_banksy_lambda1_arcOnly_res04.rds"))
...
saveRDS(spe_list, file = paste0("spe_list_banksy_lambda1_arcOnly_res04.rds"))


######################################################################### 
# Parsing BANKSY output
######################################################################### 
spe_list = readRDS("spe_list_banksy_lambda1_arcOnly_res04.rds")
use_agf <- FALSE
lambda <- 1
res <- 0.4
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
pdf("multisample_banksy_lambda1_arcOnly_res04.pdf", width = 40, height = 20)
plot_grid(plotlist = plot_bank, ncol = 3, byrow = TRUE)
dev.off()
# # plot_bank[[1]]
# 

#  still shows overlapped subdomains
