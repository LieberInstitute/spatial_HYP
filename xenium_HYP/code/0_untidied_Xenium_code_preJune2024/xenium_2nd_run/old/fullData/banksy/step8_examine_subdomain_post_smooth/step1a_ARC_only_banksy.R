...
colData(spe_joint)[,"clust_M0_lam1_k50_res0.6_smooth"]=NULL
...
saveRDS(spe_list, file = paste0("spe_list_banksy_lambda1_arcOnly_redoSmooth.rds"))


######################################################################### 
# Parsing BANKSY output
######################################################################### 
spe_list = readRDS("spe_list_banksy_lambda1_arcOnly_redoSmooth.rds")
use_agf <- FALSE
lambda <- 1
res <- 0.6
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
nclu = length(unique(colData(spe_list[[1]])[,sprintf("%s_smooth", cnm)]))
...
pdf("multisample_banksy_lambda1_arcOnly_res06_redoSmooth.pdf", width = 40, height = 20)
