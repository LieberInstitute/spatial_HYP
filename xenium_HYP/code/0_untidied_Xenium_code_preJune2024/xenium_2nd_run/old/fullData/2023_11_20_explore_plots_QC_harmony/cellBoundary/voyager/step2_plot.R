...
dir_data_processed = "/dcs04/hansen/data/ywang/xenium_newData/data_processed/"
dir_out = "/dcs04/hansen/data/ywang/xenium_newData/out/voyager/"
setwd(dir_out)
# plotSpatialFeature
###########
library(Voyager)
...
dir_allSamples = "/dcs04/lieber/marmaypag/xeniumHYP_LIBD4195/xenium_hyp/raw-data/xenium/20240223__174743__022324_KMon120823/"
list_sample = list.files(dir_allSamples)
...
# sfe = readRDS(paste0(dir_data_processed, "obj_sfe_oneSample_",sample_name,".rds"))
...
make_plot_grid<- function(fun_voyager, name_pdf, main, list_arg, 
  ...
  plist = list()
  pdf(name_pdf, width = 40, height = 20)
  for(ksample in 1:7){
    sample_name = list_sample[ksample]
    if(sfe_keep){
      sfe = readRDS(paste0(dir_data_processed, "obj_sfe_keep_oneSample_",sample_name,".rds"))
    }else{
      sfe = readRDS(paste0(dir_data_processed, "obj_sfe_oneSample_",sample_name,".rds"))
    }
    colnames(sfe) <- seq_len(ncol(sfe))
...
for(ksample in 1:7){
  ...
  saveRDS(sfe, file = paste0(dir_data_processed, "obj_sfe_oneSample_",sample_name,".rds"))
}
make_plot_grid(fun_voyager = plotColDataHistogram, 
...
for(ksample in 1:7){
  ...
  (sfe <- sfe[,inds_keep])
  saveRDS(sfe, file = paste0(dir_data_processed, "obj_sfe_keep_oneSample_",sample_name,".rds"))
}
