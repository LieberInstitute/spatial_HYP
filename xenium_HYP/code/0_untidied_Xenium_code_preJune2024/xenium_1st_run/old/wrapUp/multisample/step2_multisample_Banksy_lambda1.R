...
dir.create("/dcs04/hansen/data/ywang/xenium/wrapup/multi_sample_banksy")
...
spe_list = readRDS(paste0(dir_data_processed_banksy,"spe_list.rds"))
...
# saveRDS(spe_joint, file = paste0("spe_joint_banksy_lambda1.rds"))


saveRDS(spe_joint, file = paste0("spe_joint_lambda1_UMAP.rds"))  
...
for(res in c(0.5, 1, 1.2, 2, 4, 5)){
  ...
  # saveRDS(spe_joint, file = paste0("spe_joint_banksy_lambda1_res", res, ".rds"))
  
  
  # colData_ = colData(spe_joint)
  # # saveRDS(colData_, file = paste0( dir_data_processed_banksy,
  #                                 "colData_allSamples_preprosssed_res",
  #                                 res,"_lambda1.rds"))  
  ...
  # 
  list_colData = list()
  for(sample_tmp in names(spe_list)){
    list_colData[[sample_tmp]] = colData(spe_list[[sample_tmp]])
  }

  saveRDS(list_colData, file = paste0("list_colData_banksy_lambda1_res", res, ".rds"))
  # saveRDS(spe_list, file = paste0("spe_list_banksy_lambda1_res", res, ".rds"))
}
...
# ######################################################################### 
# spe_list = readRDS("spe_list_banksy_lambda1.rds")
# use_agf <- FALSE
# lambda <- 1
# res <- 0.6
# cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
# 
# pdf("multisample_banksy_lambda1.pdf", width = 40, height = 20)
