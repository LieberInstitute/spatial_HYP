...
saveRDS(spe_list, file = paste0(dir_data_processed_banksy,"spe_list.rds"))
...
saveRDS(spe_joint, file = paste0("spe_joint_banksy_lambda02.rds"))
...
saveRDS(spe_list, file = paste0("spe_list_banksy_lambda02.rds"))

######################################################################### 
# Parsing BANKSY output
######################################################################### 
spe_list = readRDS("spe_list_banksy_lambda02.rds")
use_agf <- FALSE
lambda <- 0.2
res <- 0.6
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
pdf("multisample_banksy_lambda02.pdf", width = 40, height = 20)
plot_grid(plotlist = plot_bank, ncol = 3, byrow = TRUE)
dev.off()
# plot_bank[[1]]

pdf("multisample_banksy_lambda02_smallPage.pdf", width = 25, height = 12)
plot_grid(plotlist = plot_bank, ncol = 3, byrow = TRUE)
dev.off()
