...
for(sample in names(spe_list_fullData)){
  ...
  # se_seurat[,cellClu=="4"]
  spe_list[[sample]] = spe_list_fullData[[sample]][,cellClu=="4"]
  
}

saveRDS(spe_list, file="spe_list_banksy_lambda1_vmhOnly.rds")
...
pdf(paste0("multisample_banksy_lambda_", lambdaName, "_vhmOnly.pdf"), width = 40, height = 60)
plt()
dev.off()
...
pdf(paste0("multisample_banksy_lambda_", lambdaName, "_largerPage_vhmOnly.pdf"), width = 80, height = 60)
plt()
dev.off()

pdf(paste0("multisample_banksy_lambda_", lambdaName, "_smallerPage_vhmOnly.pdf"), width = 20, height = 30)
plt()
dev.off()
