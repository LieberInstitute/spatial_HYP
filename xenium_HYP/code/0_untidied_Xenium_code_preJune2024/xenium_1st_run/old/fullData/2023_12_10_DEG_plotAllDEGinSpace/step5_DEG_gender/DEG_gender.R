...
pdf(paste0("DE_gender_vhm_multisampleBanksy_lambda_",lambdaName,"_allDEG.pdf"), width = 20, height = 20)
...
pdf(paste0("DE_gender_arc_multisampleBanksy_lambda_",lambdaName,"_allDEG.pdf"), width = 20, height = 20)
######## format se into seurat format
obj = CreateSeuratObject(assays(se)$counts[, cellClu == cellClu_used], 
                         project = paste0("marker genes"))
obj = NormalizeData(obj)
...
set.seed(111)#plot a subset of cells to reduce running time
rd_cells = sample(1:ncol(obj), 10000)
print(DoHeatmap(obj[DEG$gene, rd_cells] , features = top10$gene) + NoLegend())
