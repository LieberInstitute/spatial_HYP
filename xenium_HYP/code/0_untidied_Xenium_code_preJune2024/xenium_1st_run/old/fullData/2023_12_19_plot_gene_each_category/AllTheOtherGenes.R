...
genes_plotted_before = rbind(genes_ARC_sexDEG,
                             genes_ARC_marker,
                             genes_VMH_marker,
                             genes_VMH_sexDEG[-1,])

genes_others = geneInfo_[!geneInfo_[,1]%in%genes_plotted_before[,1],]
genes_others = genes_others[-1,]
...
rownames(genes_others) = genes_others[,1]

pdf("AllTheOtherGenes_spatialPlot_boxPlot.pdf",width=30,height=30)
# for(gene_tmp in intersect(genes_others[,1] , rownames(spe))){
for(gene_tmp in intersect(genes_others[,1] , rownames(spe))){
  ...
  p <- ggplot(df, aes(x = x_location, y = y_location, color = expr)) + 
    ...
    ggtitle(paste0(gene_tmp, ", ", genes_others[,2])) + 
    theme_bw() + 
