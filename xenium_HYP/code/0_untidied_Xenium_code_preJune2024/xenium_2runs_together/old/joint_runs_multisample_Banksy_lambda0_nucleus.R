...
# /dcs04/hansen/data/ywang/xenium_newData/data_processed_banksy/
dim(spe_lists_old[[1]])
dim(spe_list_new[[1]])
...
######################################################################### 
spe_list = readRDS("spe_list_2runs_banksy_lambda0_nucleus.rds")
use_agf <- FALSE
lambda <- 0
res <- 0.6
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
pdf("multisample_2runs_banksy_lambda0_nucleus.pdf", width = 60, height = 100)
