...
saveRDS(spe_joint, file = paste0("spe_joint_banksy_lambda1_13samples.rds"))
...
saveRDS(spe_list, file = paste0("spe_list_banksy_lambda1_13samples.rds"))

######################################################################### 
# Parsing BANKSY output
######################################################################### 
spe_list = readRDS("spe_list_banksy_lambda1_13samples.rds")
use_agf <- FALSE
lambda <- 1
res <- 0.6
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
pdf("multisample_banksy_lambda1_13samples.pdf", width = 40, height = 20)
