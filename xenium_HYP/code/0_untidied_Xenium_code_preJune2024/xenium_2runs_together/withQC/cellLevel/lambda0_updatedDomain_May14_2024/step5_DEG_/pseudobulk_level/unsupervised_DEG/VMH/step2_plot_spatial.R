...
dir.create("ARC_DEG")
gene_tmp = "ESR1"
for(cellClu_tmp in names(list_DEG_perClu)[1]){
  print(cellClu_tmp)
  
  dir.create(paste0("ARC_DEG/clu", cellClu_tmp))
  genes_deg_tmp = list_DEG_perClu[[cellClu_tmp]]
  ...
  for(gene_tmp in genes_deg_tmp){
    ...
    pdf(paste0("ARC_DEG/clu", cellClu_tmp,"/",gene_tmp,".pdf"),width=40,height=40)
...
############# plot spatial plot, for presentation
gene_tmp = "CDH13"
...
range_exp_geneTmp = range(df[,gene_tmp])
# range_exp_geneTmp = c(0,5)
pdf(paste0("ARC_DEG/clu", cellClu_tmp,"/",gene_tmp,"__.pdf"),width=40,height=20)

p1 <- ggplot(df_cellType_clu_tmp, aes(x = x_location, y = y_location, color = expr)) +
  geom_point(size = .0005) +
  coord_fixed() +
  scale_fill_continuous(limits = c(range_exp_geneTmp))+
  scale_color_gradient(low = "darkblue", high = "red",
                       trans = "sqrt", breaks = c(0,4),
                       name = "expression level") +
  ggtitle(paste0(gene_tmp,", cluster ", cellClu_tmp)) + 
  theme_bw() +
  theme(plot.title = element_text(face = "italic"),
  ...
  facet_wrap(~sex + sample_id, ncol = 7)+
  coord_equal()

p2 <- ggplot(df_cellType_clu_tmp_back, aes(x = x_location, y = y_location, color = expr)) +
  geom_point(size = .0005) +
  coord_fixed() +  scale_fill_continuous(limits = c(range_exp_geneTmp))+
...
print(p1)
print(p2)
# print(ggarrange(p1, p2, heights = c(3, 3),
                # ncol = 1, nrow = 5))
dev.off()
