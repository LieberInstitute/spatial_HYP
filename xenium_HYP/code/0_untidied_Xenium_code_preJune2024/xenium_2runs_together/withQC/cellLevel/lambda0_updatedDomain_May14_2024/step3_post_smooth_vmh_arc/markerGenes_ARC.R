...
spe_joint  = readRDS(paste0("spe_joint_2runs_banksy_lambda",lambdaName,".rds"))
se = spe_joint
...
# spe = se[,cellClu=="4"]
# cellClu_used = "4"
# if_ARC = ifelse(cellClu=="7", "ARC", "not ARC")
...
spe = se[,sele]
# spe = 
pdf("AnnotatedARCMarker_spatialPlot_boxPlot_ARC.pdf",width=30,height=30)
for(gene_tmp in intersect(genes_ARC_marker[,1] , rownames(spe))){
  df <- as.data.frame(
  ...
  df$ARC = factor(df$ARC, levels = c("ARC", "not ARC"))
  # df[df$sex!="Male",]
  p_box <- ggplot(df, aes(x = sex_sampleID, y = expr, fill =  ARC)) + 
    ggtitle(gene_tmp) + 
    theme_bw() + 
