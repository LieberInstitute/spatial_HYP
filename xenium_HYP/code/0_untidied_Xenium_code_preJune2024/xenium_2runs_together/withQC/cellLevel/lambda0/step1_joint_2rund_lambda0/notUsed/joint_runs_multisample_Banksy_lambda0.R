...
saveRDS(spe_list, file = paste0("spe_list_2runs_banksy_lambda0.rds"))
print("44")

######################################################################### 
# Parsing BANKSY output
######################################################################### 
spe_list = readRDS("spe_list_2runs_banksy_lambda0.rds")
use_agf <- FALSE
lambda <- 0
res <- 0.6
cnm <- sprintf("clust_M%s_lam%s_k50_res%s", as.numeric(use_agf), lambda, res)
...
pdf("multisample_2runs_banksy_lambda0.pdf", width = 60, height = 100)
