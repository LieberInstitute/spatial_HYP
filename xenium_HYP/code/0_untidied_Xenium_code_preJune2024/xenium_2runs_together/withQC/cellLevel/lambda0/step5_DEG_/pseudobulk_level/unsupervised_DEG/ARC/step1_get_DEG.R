...
for(cell_clu in cellClu_kp){
  ...
  ####################
  pseudobulk_clu_tmp =  (lapply(SampleIDs_kp,function(xx){
    rowMeans(logcounts_tmp[,which(colData(spe_tmp)$sample_id == xx)])
  }))
  
  
  
  pseudobulk_clu_tmp = matrix(unlist(pseudobulk_clu_tmp), ncol = length(pseudobulk_clu_tmp))
  colnames(pseudobulk_clu_tmp) = SampleIDs_kp
  rownames(pseudobulk_clu_tmp) = rownames(spe_tmp)
    
  ####################
  which_m = sex_perSampleID[SampleIDs_kp]=="Male"
  which_f = sex_perSampleID[SampleIDs_kp]=="Female"
...
# cp /dcs04/hansen/data/ywang/xenium_joint_runs/out/list_*_allGenes_perClu_pseudobulkLevel_ARC.rds ./
