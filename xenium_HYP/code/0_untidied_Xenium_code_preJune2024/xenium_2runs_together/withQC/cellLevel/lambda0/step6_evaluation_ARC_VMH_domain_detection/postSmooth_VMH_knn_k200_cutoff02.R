...
# pred_vmh = (knn_pred_vmh_c>cutoff)
...
for(sample in sample_names){
  ...
  # load results of smoothed domain after 2 rounds of knn smoothing
  knn_pred = readRDS(paste0("knn_predVMH_",sample,"_k200_2ndRun.rds"))
  pred_vmh_which = which(knn_pred>cutoff)
  ...
  vec_cells_knn = unique(as.integer(knn_$nn.index[pred_vmh_which,]))
  
  vec_cells_knn_outsideDomain = vec_cells_knn[!vec_cells_knn  %in% pred_vmh_which]
  vec_cells_insideDomain = pred_vmh_which
  vec_cells_others = c(1:length(which_sample))[-c(vec_cells_knn_outsideDomain,pred_vmh_which)]
  saveRDS(vec_cells_knn_outsideDomain, file = paste0("vec_cells_knn_outsideVMH_",sample,"_k500_2ndRun.rds") )
  saveRDS(vec_cells_insideDomain, file = paste0("vec_cells_insideVMH_",sample,"_k500_2ndRun.rds") )
  saveRDS(vec_cells_others, file = paste0("vec_cells_others_",sample,"_k500_2ndRun.rds") )
  
  # knn_pred = unlist(lapply(1:length(which_sample), function(xx){
  # mean(knn_pred_vmh_c[which_sample[knn_$nn.index[xx,]]])
  # }))
...
for(sample in sample_names){
  ...
  vec_cells_knn_outsideDomain = readRDS(paste0("vec_cells_knn_outsideVMH_",sample,"_k500_2ndRun.rds"))
  vec_cells_insideDomain = readRDS(paste0("vec_cells_insideVMH_",sample,"_k500_2ndRun.rds"))
  vec_cells_others = readRDS(paste0("vec_cells_others_",sample,"_k500_2ndRun.rds"))
...
# do the plots - boxplt
df_ = data.frame(expr = c(vec_allSamples_exp_knn_outsideDomain, vec_allSamples_exp_insideDomain, vec_allSamples_exp_others),
...
pdf("validation_vmh_domain_detection.pdf", width = 15)
...
plt <- function(se_, cnames, 
  ...
  plot_bank1 <- plotColData(se_[,grp1],
    ...
    scale_shape_manual(values=seq(0,3))
...
# vec_allSamples_which_knn_outsideDomain = c(vec_allSamples_which_knn_outsideDomain, which_sample[vec_cells_knn_outsideDomain] )
# vec_allSamples_which_insideDomain = c(vec_allSamples_which_knn_outsideDomain, which_sample[vec_cells_insideDomain] )
# vec_allSamples_which_others = c(vec_allSamples_which_knn_outsideDomain, which_sample[vec_cells_others] )
# df_$group = factor(df_$group, levels = c("within_domain","near_domain","far_from_domain"))
...
group_ = factor(as.character(group_), levels = c("within_domain","near_domain","far_from_domain"))
# group_
...
pdf("validation_vmh_domain_detection_spatial.pdf", width = 60, height = 100/2)
plt(se[,], cnames = "group")
dev.off()
