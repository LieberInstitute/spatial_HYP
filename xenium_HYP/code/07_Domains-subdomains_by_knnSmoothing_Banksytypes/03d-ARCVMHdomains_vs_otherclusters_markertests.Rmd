---
title: "03d-ARCVMHdomains_vs_otherclusters_markertests"
...
### load SFE, drop discarded clusters, apply domain labels, label VMH and ARC domains as singularized "cell types", drop cells that are in VMH/ARC clusters but outside the domains, apply labels from scripts 03a/03b
```{r}
...
# annots - remove DISCARD clusters
...
# merge in domain labels and overwrite cluster labels where domain is ARC or vmh
bkcl <- merge.data.table(bkcl,arcvmh4,by="rn")
bkcl[dualVMHARC4=="VMH",clus:="VMH"]
bkcl[dualVMHARC4=="ARC",clus:="ARC"]

# drop VMH/ARC cells that aren't in the domains
bkcl <- bkcl[!(bjm_annot %in% grep(bjm_annot,pattern="VMH|ARC",value=T)&dualVMHARC4=="other")]

# subset spe to these cells
hypb <- hypb[,bkcl$rn]

# apply labels
bkcl <- DataFrame(bkcl,row.names=bkcl$rn)[colnames(hypb),]
colLabels(hypb) <- bkcl$clus
```


### run one-v-all marker analyses, where ARC and VMH domain contents are now
```{r}
dommk <- registration_wrapper(hypb,var_registration="label",var_sample_id = "sample_id",gene_ensembl = "ID",gene_name = "Symbol")



## save
saveRDS(dommk,"processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/03d-Allcellsof-ARCVMH-in-singeclusters-otherARCVMHcellsdropped_1vallReprodClus-markers.RDS")
```
...
[15] matrixStats_1.3.0              SpatialFeatureExperiment_1.6.1
