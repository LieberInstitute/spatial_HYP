---
title: "01a-Broad VMH domain by knn (first smoothing)"
...
## adapting from script (despite filename, is specifically VMH step 1 smoothing): 
../../0_untidied_Xenium_code_preJune2024/xenium_2runs_together/withQC/cellLevel/lambda0_updatedDomain_May14_2024/step3_post_smooth_vmh_arc/step1_knn/postSmooth_ARC_VMH_knn.R
...
### adapting yi's code, make a table of x-y coordinates and VMH (yes/no) assignment. only use the three VMH clusters that are found across the dataset (X12, X18, X30). in order to perform smoothing on each sample individually, split the resulting table into a list of tables, one table per sample.
...
## make a data table of only the essential columns for smoothing and later return of these results to the SPE: namely, the x y coords, VMH status (one of the VMH clusters or not), and unique cell id (==colnames(hypx)) 
xyvmh <- as.data.table(colData(hypx),keep.rownames=T)
xyvmh[,isVMH:=0]
xyvmh[banksy_m0l0kg6_res2 %in% c("X12","X18","X30"),isVMH:=1]
sum(xyvmh$isVMH==1)
# 67217 cells total
...
xyvmh <- xyvmh[,.(rn,sample_id,xcoord,ycoord,isVMH)]

## make samplewise table list
xyvmh_list <- split(xyvmh,xyvmh$sample_id)
...
## step 1 knn smoothing of VMH with k=10
...
#### however, this approach is more transparent in that the user can take an entry of xyvmh_list into the environment as v and step through the bplapply to see what's being done here, and can see that cells are always matched in the process (as opposed to working with vectors where we don't have cell ids matched up anymore. which always makes me unccomfortable, however rationally/irrationally.)
```{r}
knnreslist <- bplapply(xyvmh_list,BPPARAM=MulticoreParam(10),FUN=function(v){
    ...
    # make the list names the cell identifiers as a failsafe against reordering
    names(neighborslist) <- v$rn
    
    # now, get the mean of VMH y/n for the k nearest neighbors indexed for each cell
    # (in knnout, the first list item, nn.index, has v's row indices corresponding to those neighbors) 
    # this value is thus a probability 0-1 that, a given cell is a VMH type given the VMH/nonVMH identity of its 10 spatial neighbors
...
xyvmhout <- rbindlist(knnreslist)

## save the resulting smoothing predictions as a table of cell ID, coords, and VMHknn smoothing step 1 prediction values
fwrite(xyvmhout,"processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/01a-step1smoothing_VMH_knn-k10.txt",sep='\t',quote=F)
```
