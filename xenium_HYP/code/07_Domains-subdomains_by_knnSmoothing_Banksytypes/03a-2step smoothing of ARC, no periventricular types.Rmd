...
### here we will re run both steps of the ARC domain smoothing, but without the primary cell types that extends far into the periventricular region (X23, X19)
### this still requires co-smoothign of VMH to resolve ambiguous labeling
```{r}
...
make a table of x-y coordinates and ARC (yes/no) assignment. only use the FOUR clusters that are predominantly in ARC and are found across the donors (X15, X19 X31, X33); X23 is dense throughout periV across samples (X19 has some as well but is densest in ARC proper, where we want to get the domain smoothed). in order to perform smoothing on each sample individually, split the resulting table into a list of tables, one table per sample.

we'll use all three data-wide VMH clusters for the VMH part: X12, X18, X30
```{r}
...
xyARC4 <- as.data.table(colData(hypx),keep.rownames=T)
xyARC4[,isARC:=0]
xyARC4[banksy_m0l0kg6_res2 %in% c("X15","X31","X19","X33"),isARC:=1]
xyARC4[,isVMH:=0]
xyARC4[banksy_m0l0kg6_res2 %in% c("X12","X18","X30"),isVMH:=1]
...
xyARC4 <- xyARC4[,.(rn,sample_id,xcoord,ycoord,isARC,isVMH)]

## make samplewise table list
xyARC_list <- split(xyARC4,xyARC4$sample_id)
...
### apply Yi's threshold and params for round 1 ARC smoothing (k=50, thresh >0.1) and for round 1 VMH smoothing (k=10, thresh = 0.2)
```{r}
knnreslist <- bplapply(xyARC_list,BPPARAM=MulticoreParam(10),FUN=function(v){
    ...
    ## repeat for VMH with k = 10
    knnout <- FNN::get.knn(v[,c("xcoord","ycoord")],k = 10)
    neighborslist <- apply(knnout$nn.index,MARGIN=1,FUN=list)
    neighborslist <- lapply(neighborslist,FUN=unlist)
    names(neighborslist) <- v$rn
    
    v[,VMHpred.smooth1.k10:=
          apply(.SD, MARGIN = 1, FUN=function(x){
              mean(v[neighborslist[[x]],isVMH])}
              )
      ,.SDcols = "rn"]
...
## label cells meeting the ARC smoothing round 1 threshold (0.1), the VMH round 1 threshold (0.2), and deconflict dual-labelled cells from this step
xyARCout[,dualVMHARC4smooth1:="other"]
xyARCout[VMHpred.smooth1.k10<=0.2&ARCpred.smooth1.k50>0.1,dualVMHARC4smooth1:="ARC"]
xyARCout[VMHpred.smooth1.k10>0.2&ARCpred.smooth1.k50<=0.1,dualVMHARC4smooth1:="VMH"]
xyARCout[VMHpred.smooth1.k10>0.2&ARCpred.smooth1.k50>0.1,dualVMHARC4smooth1:=ifelse(VMHpred.smooth1.k10>ARCpred.smooth1.k50,yes="VMH",no="ARC")]

rm(knnreslist,xyARC_list)
gc()
```

## now we want to smooth the above result using k=500 neighbors
```{r}
xyARCout[,isARCsmooth1:=0]
xyARCout[,isVMHsmooth1:=0]
xyARCout[dualVMHARC4smooth1=="ARC",isARCsmooth1:=1]
xyARCout[dualVMHARC4smooth1=="VMH",isVMHsmooth1:=1]
...
knnreslist2 <- bplapply(xyARC_list,BPPARAM=MulticoreParam(10),FUN=function(v){
    ...
    ### repeat for VMH w k=200
    knnout <- FNN::get.knn(v[,c("xcoord","ycoord")],k = 200)
    neighborslist <- apply(knnout$nn.index,MARGIN=1,FUN=list)
    neighborslist <- lapply(neighborslist,FUN=unlist)
    names(neighborslist) <- v$rn
    
    v[,VMHpred.smooth2.k200:=
          apply(.SD, MARGIN = 1, FUN=function(x){
              mean(v[neighborslist[[x]],isVMHsmooth1])}
              )
      ,.SDcols = "rn"]
    return(v)
...
## add first round threshold info to colnames
setnames(xyARCout,"VMHpred.smooth2.k200","VMHpred_thresh1_0.2_smooth2.k200")
setnames(xyARCout,"ARCpred.smooth2.k500","ARCpred_thresh1_0.1_smooth2.k500")
```

## save the KNN calcs
```{r}
fwrite(xyARCout,"processed-data/07_Domains-subdomains_by_knnSmoothing_Banksytypes/03a-Alt_2stepKNN-ARCVMH_ARC-no-X23.txt",sep='\t',quote=F,row.names=F,col.names=T)
```
...
[23] KernSmooth_2.23-24        abind_1.4-5              
...
[39] DelayedMatrixStats_1.26.0 spdep_1.3-5              
[41] scuttle_1.14.0            DBI_1.2.3                
[43] proxy_0.4-27              rhdf5_2.48.0             
[45] zlibbioc_1.50.0           assertthat_0.2.1         
[47] parallel_4.4.1            s2_1.1.6                 
[49] tiff_0.1-12               boot_1.3-30              
[51] pdist_1.2.1               Matrix_1.7-0             
[53] spData_2.3.1              jsonlite_1.8.8           
[55] fftwtools_0.9-11          BiocNeighbors_1.22.0     
[57] jpeg_0.1-10               magick_2.8.3             
[59] locfit_1.5-9.10           limma_3.60.3             
[61] units_0.8-5               DropletUtils_1.24.0      
[63] codetools_0.2-20          deldir_2.0-4             
[65] UCSC.utils_1.0.0          htmltools_0.5.8.1        
[67] rhdf5filters_1.16.0       GenomeInfoDbData_1.2.12  
[69] R6_2.5.1                  wk_0.9.1                 
[71] sfheaders_0.4.4           sparseMatrixStats_1.16.0 
[73] rprojroot_2.0.4           evaluate_0.24.0          
[75] lattice_0.22-6            R.methodsS3_1.8.2        
[77] png_0.1-8                 class_7.3-22             
[79] Rcpp_1.0.12               SparseArray_1.4.8        
[81] xfun_0.45                
> sessioninfo::session_info()
─ Session info ────────────────────────────────────────────────────────────────
 setting  value
