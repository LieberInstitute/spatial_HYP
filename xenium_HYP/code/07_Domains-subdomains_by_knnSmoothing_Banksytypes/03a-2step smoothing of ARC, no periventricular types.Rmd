...
make a table of x-y coordinates and ARC (yes/no) assignment. only use the THREE clusters that are predominantly in ARC and are found across the donors (X15,  X31, X33); X23 is dense throughout periV across samples (X19 has some as well but is densest in ARC proper, where we want to get the domain smoothed). in order to perform smoothing on each sample individually, split the resulting table into a list of tables, one table per sample.
