...
Also examine the individual clusters (super hi-res level of hierarchy) sex differentially abundant clusters for correlation to sex DE
```{r}
# 1. 5 are small, sex-specific clusters: clusters 211, 1402, 2536 and 2538 are male-specific and cluster 2207 is female-specific.
# 2. 23 sex-dominant clusters include
...
bothhyp2 <- bothhyp[,bothhyp$cluster %in% grep(bothhyp$cluster,pattern="^x211_|1402|2536|2538|2207|1404|2058|2062|2065|2088|2089|2154|2196|2204|3612|1396|1407|1409|1781|1843|2048|2057|2061|2150|2195|3359|3716|3952",value=T)]

dim(bothhyp2)

keepclusts <- as.data.table(table(bothhyp2$library_label.x,bothhyp2$cluster))[N>4,.N,by="V2"]$V2

bothhyp2 <- bothhyp2[,bothhyp2$cluster %in% keepclusts]
...
proc <- dreamlet::aggregateToPseudoBulk(bothhyp2,sample_id="library_label.x",cluster_id = "cluster",BPPARAM = sbp)

dif <- dreamlet::processAssays(proc,min.cells = 5,min.count = 1,min.samples = 4,min.prop = 0.5,normalize.method = "TMM",useCountsWeights = T,BPPARAM = sbp,formula = ~donor_sex)
...
saveRDS(sexdif2,"analysis/data/spe_053123/H07-Outside Dataset Analyses and Comparisons/01-ABA-Yao23-HYPscell-sexdiffabundantclusters_dreamlet-sexDE.RDS")
