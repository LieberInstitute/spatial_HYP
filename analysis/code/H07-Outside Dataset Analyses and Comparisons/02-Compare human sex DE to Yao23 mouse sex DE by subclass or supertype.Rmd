...
get results and cell types of interest
```{r}
...
subcs <- unique(grep(unlist(abasex[[1]]$assay),pattern="VMH|DMH|ARH|PV|AHN|LHA|PVH|Tanycyte",value=T))
supts <- unique(grep(unlist(abasex[[2]]$assay),pattern="VMH|DMH|ARH|PV|AHN|LHA|PVH|Tanycyte",value=T))
```
...
colData(hyp2) <- tmpcd
colData(hyp2)$Sex.x <- as.factor(colData(hyp2)$Sex.x)
...
dif <- dreamlet::processAssays(proc,min.cells = 10,min.count = 1,min.samples = 4,min.prop = 0.4,normalize.method = "TMM",useCountsWeights = T,BPPARAM = sbp,formula = ~Sex.x)

sexdif <- dreamlet::dreamlet(dif,formula = ~Sex.x,BPPARAM = sbp)

# find relevant coefficient name
coefNames(sexdif)
#
sexdif2 <- dreamlet::topTable(sexdif,coef="Sex.xM",number=Inf,adjust.method ="BH")
