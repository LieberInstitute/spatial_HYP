---
title: "01-Graph-based Harmony Cluster Markers"
...
walktrap.markers <- bplapply(walktraps,BPPARAM=bpparam,FUN=function(x){
    # row names as gene symbols for sanity
    rownames(hyp2) <- rowData(hyp2)$gene_name
    colLabels(hyp2) <- factor(x)
    mks <- findMarkers(hyp2,test="binom",direction="up")
})
names(walktrap.markers) <- names(walktraps)

# saveRDS(walktrap.markers,"analysis/data/H03-marker_genes/sharmony_walktrapk5-15_markergenes.RDS")
```

### Extract top FDR gene(s) per cluster per cluster set into plaintext tabular format
