...
precast_rdses <- list.files("analysis/data/H02-clustering/04-precast_hvgsvg/precast_hvg-svg_out",pattern=".RDS",full.names = T)
...
for (i in c(1:length(precast_rdses))){
    ...
    precast_list[[(length(precast_list)+1)]] <- clustdt
    names(precast_list)[length(precast_list)] <- gsub(precast_rdses[i],pattern="^.*hvg-svg_out\\/(.*_k.*)_precast.*$",replacement="\\1")
    rm(clustdt)
}

# check that the # of clusters assigned are corrected for each table
lapply(precast_list,FUN=function(x){length(unique(x$label))})
# oops, fix the list names
names(precast_list) <- gsub(names(precast_list),pattern="^/(.*)$",replacement="\\1")

# saveRDS(precast_list,"analysis/data/H02-clustering/04c-PRECAST_clusters_hvg1020_nnsvg1020_k9-15-20-31.RDS")
```
