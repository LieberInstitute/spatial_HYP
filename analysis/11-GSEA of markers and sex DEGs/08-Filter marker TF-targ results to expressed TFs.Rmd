...
mkreslist <- readRDS("data/11-GSEA/02a-Marker TF-target GSEA svg10-hmnyLmdaNA_BS60k-k15-k20-k31-k15clpsd.RDS")
...
# VMH and ARC were labeled XVMH / XARC in this analysis
bscls[[4]][label %in% c("X11","X12"),label:="XVMH"]
bscls[[4]][label %in% c("X3","X5"),label:="XARC"]

### but note that X14 and X15 from the collapsed clustering didn't have results calculated.
...
mkreslist.filt <- bpmapply(R=mkreslist,C=bscls,N=as.list(names(bscls)),BPPARAM=sbp,FUN=function(R,C,N){
    ...
    for (i in c(1:length(R))){
        ...
        # exclude collapsed X14, X15, for which results weren't calculated
        if(curclus %in% c("k15_HARMONYlmbna_nnsvg10_60kiter_collapsed_X14","k15_HARMONYlmbna_nnsvg10_60kiter_collapsed_X15")){next(i)}
        ...
        # FINALLY  get the TFs meeting criteria (majority of samples expressing in > 0.25% of spots and >1% of spots in the clustering expressing on average across the dataset) and filter the result table to them
        
        keeptfs <- tffilt2[Nxpr>4&meanxpr>0.01,tf]
...
saveRDS(megalist.filt,"data/11-GSEA/08-Marker TF-target GSEA results from 02a filtered to TFs expressed in avg 1pct of cluster spots and at least .25pct of cluster spots in 5samps.RDS")
```
