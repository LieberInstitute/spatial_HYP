...
mkreslist.filt <- bpmapply(R=mkreslist,C=bscls,N=as.list(names(bscls)),BPPARAM=sbp,FUN=function(R,C,N){
    ...
    for (i in c(1:length(R))){
        ...
        # FINALLY  get the TFs meeting criteria (majority of samples expressing in >3% of spots and >5% of spots in the clustering expressing on average across the dataset) and filter the result table to them
...
saveRDS(megalist.filt,"data/11-GSEA/08-Marker TF-target GSEA results from 02a filtered to TFs expressed in avg 3pct of cluster spots and at least 1pct of cluster spots in 5samps.RDS")
```
