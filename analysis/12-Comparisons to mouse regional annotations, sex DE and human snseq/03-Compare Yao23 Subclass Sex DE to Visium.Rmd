...
vde <- lapply(vde,FUN=function(x){x[gene_name %in% msgenes$ortholog_gene & !(chromosome_name %in% c("chrX","chrY","chrMT"))]})
...
vde <- lapply(vde,FUN=function(x){merge(x,msgenes,by.x="gene_name",by.y="ortholog_gene")})
```
...
restabs <- bpmapply(x=vde,y=cluspairs,SIMPLIFY = F, BPPARAM=sbp,FUN=function(x,y){
    ...
    for (i in c(1:nrow(y))){
        ...
        tmpms <- copy(ms)[assay==msassay][,.(t,ortholog_gene)]
        tmpx <- x[assay==hgassay][,.(t,gene_name)]
        setnames(tmpms,"t","tms")
        
        tmpcompare <- merge.data.table(tmpms,tmpx,by.x="ortholog_gene",by.y="gene_name")
