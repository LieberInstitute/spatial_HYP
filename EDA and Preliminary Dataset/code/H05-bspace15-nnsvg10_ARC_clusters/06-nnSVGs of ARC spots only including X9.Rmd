...
res_list3 <- lapply(res_list2,FUN=function(x){
    ...
    as.data.table(as.data.frame(rowData(x)),keep.rownames=F)[nnSVG_padj<0.2]
})
names(res_list3) <- caps
#store,save, clean up
saveRDS(res_list3,"analysis/data/spe_053123/H05-bspace15-nnsvg10_ARC_clusters/06-nnSVG-3-1pt5_ARC8-9-13-spots-only_FDRpt2-per-samp_postcropD07075B1.RDS")
rm(i,caps,sbp,res_list2)

### get mean ranks
i<-1
for (i in c(1:sum(unique(colData(hyp2)$sample_id)%in%names(res_list3)))){
  if(i==1){
  nnsvg_ranks <- res_list3[[i]][nnSVG_padj<0.05]
  nnsvg_ranks <- nnsvg_ranks[,.(gene_id,gene_name,nnSVG_rank)]
  setnames(nnsvg_ranks,3,names(res_list3)[i])
  }
  else{
  tmp <- res_list3[[i]][nnSVG_padj<0.05]
  tmp <- tmp[,.(gene_id,gene_name,nnSVG_rank)]
  setnames(tmp,3,names(res_list3)[i])
...
write.table(nnsvg_ranks,"analysis/data/spe_053123/H05-bspace15-nnsvg10_ARC_clusters/06-meangeneranks_nnSVG_samplewise_n3s1.5_ARC8-9-13-spotsonly_postcropD07075B1.txt",sep='\t',quote=F,row.names=F,col.names=T)
