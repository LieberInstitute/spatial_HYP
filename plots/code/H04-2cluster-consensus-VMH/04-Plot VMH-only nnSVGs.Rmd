...
for (i in c(1:length(vmhsvgs))){
    curgene <- vmhsvgs[i]
    colLabels(hyp2.vmh) <- logcounts(hyp2.vmh)[curgene,]
    plt <- plotSpots(hyp2.vmh,annotate = "label",palette=viridis::viridis(n=1))
    png(paste0("plots/H04-2cluster-consensus-VMH/04-VMHonly-svgs/",curgene,".png"),width=800,height=800)
    print(plt)
    dev.off()
}
