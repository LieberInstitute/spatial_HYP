...
for (i in c(1:16)){
    ...
    rm(y)
    plt <- plotGroupedHeatmap(hyp2,features = mks,group="label",exprs_values = "counts")
    dev.off()
    pdf(paste0("plots/H03-marker_genes/03-BSpace_AffinatiVMHNeuronGenes/heatmaps/BSpace_",curname,"_AffinatiGenes.pdf"),width=8,height=12)
    print(plt)
    dev.off()
    rm(plt,curname)
}
...
for (i in c(1:length(mks))){
    curname <- mks[i]
    colLabels(hyp2) <- counts(hyp2)[mks[i],]
    plt <- plotSpots(hyp2,annotate = "label",palette = viridis::viridis(n=1))
    png(paste0("plots/H03-marker_genes/03-BSpace_AffinatiVMHNeuronGenes/spotplots/",curname,"_fromAffinatiGenes.png"),width=800,height=800)
    print(plt)
    dev.off()
    rm(plt,curname)
}
