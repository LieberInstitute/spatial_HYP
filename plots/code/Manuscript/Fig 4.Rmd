...
vmhvol <- ggplot(sexderes[assay=="VMH"],aes(y=-log10(P.Value),x=logFC))+
    geom_point(size=0.9,col="#001859")+
...
arcvol <- ggplot(sexderes[assay=="ARC"],aes(y=-log10(P.Value),x=logFC))+
    geom_point(size=0.9,col="#058a00")+
    geom_hline(yintercept=-log10(0.05),linetype="dashed",color="black")+
...
arcvmhcor <- ggplot(arcvmh,aes(x=logFC_VMH,y=logFC_ARC))+
    ...
    geom_text(aes(x=-1.4,y=2.6),label="Pearson Coef=0.514",size=2.5)+
    xlim(-2.6,2.6)+
    ylim(-2.6,2.6)+
    xlab("logFC VMH")+
    ylab("logFC ARC")+
...
D-E: HRH1 in ARC (FDR 0.02)
F-G: RXRG in ARC (FDR 0.09)
```{r}
hrhM <- hyp2[,hyp2$sample_id=="V12Y31-080_A1"]
rownames(hrhM) <- rowData(hrhM)$gene_name
colData(hrhM)$`log counts` <- logcounts(hrhM)["HRH1",]
hrhlab <- copy(bscl.vmharc)[rn %in% colnames(hrhM)]
hrhlab <- DataFrame(hrhlab)
rownames(hrhlab) <- hrhlab$rn

hrhM$Domain <- factor(hrhlab$cl2,levels=c("VMH","ARC"))
### make spotplots
p <- make_escheR(hrhM)
p <- p |> add_fill("log counts",size=0.25,point_size = 0.6)
p <- p |> add_ground(var = "Domain",stroke=0.15,point_size = 0.6)

pdf("plots/Manuscript/4D-HRH1_ARCsexDE_M.pdf",height=1.75,width=2)
p+
...
### 
hrhF <- hyp2[,hyp2$sample_id=="V12D05-350_C1"]
rownames(hrhF) <- rowData(hrhF)$gene_name
colData(hrhF)$`log counts` <- logcounts(hrhF)["HRH1",]
hrhlab <- copy(bscl.vmharc)[rn %in% colnames(hrhF)]
hrhlab <- DataFrame(hrhlab)
rownames(hrhlab) <- hrhlab$rn

hrhF$Domain <- factor(hrhlab$cl2,levels=c("VMH","ARC"))
### make spotplots
p <- make_escheR(hrhF)
p <- p |> add_fill("log counts",size=0.25,point_size = 0.6)
p <- p |> add_ground(var = "Domain",stroke=0.15,point_size = 0.6)

pdf("plots/Manuscript/4E-HRH1_ARCsexDE_F.pdf",height=1.75,width=2)
p+
    scale_color_manual(values=c("purple","darkgreen"),na.value = NA)+
    scale_fill_gradient(low="white",high="black")+
    ggtitle("HRH1-Female")+
...
pdf("plots/Manuscript/4F-RXRG_ARCsexDE_M.pdf",height=1.75,width=2)
p+
    scale_color_manual(values=c("purple","darkgreen"),na.value = NA)+
    scale_fill_gradient(low="white",high="black")+
    ggtitle("RXRG-Male")+
...
pdf("plots/Manuscript/4G-RXRG_ARCsexDE_F.pdf",height=1.75,width=2)
p+
    scale_color_manual(values=c("purple","darkgreen"),na.value = NA)+
    scale_fill_gradient(low="white",high="black")+
    ggtitle("RXRG-Female")+
