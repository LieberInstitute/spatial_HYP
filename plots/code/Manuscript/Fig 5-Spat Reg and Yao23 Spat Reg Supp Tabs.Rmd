...
arcplt <- list()
arcplt[[1]] <- ggplot(arcplots[sphyp_domain=="ARC.1"],aes(x=log10(`Top n Human Domain Markers`),y=`PCC (marker t stats)`,col=Subclass))+
...
arcplt[[2]] <- ggplot(arcplots[sphyp_domain=="ARC.2"],aes(x=log10(`Top n Human Domain Markers`),y=`PCC (marker t stats)`,col=Subclass))+
    geom_line(size=0.7,alpha=0.7)+
    scale_color_manual(values=arcpal)+
    ggtitle("ARC.2")+
    ylim(-0.25,0.5)+
...
pdf("plots/Manuscript/5Bi-ARC.1, ARC.2 to ARH subclasses.pdf",height=3,width=3.75)
...
vmhplt <- list()
vmhplt[[1]] <- ggplot(vmhplots[sphyp_domain=="VMH.1"],aes(x=log10(`Top n Human Domain Markers`),y=`PCC (marker t stats)`,col=Subclass))+
    geom_line(size=0.7,alpha=0.7)+
...
vmhplt[[2]] <- ggplot(vmhplots[sphyp_domain=="VMH.2"],aes(x=log10(`Top n Human Domain Markers`),y=`PCC (marker t stats)`,col=Subclass))+
    geom_line(size=0.7,alpha=0.7)+
    scale_color_manual(values=vmhpal)+
    ggtitle("VMH.2")+
    ylim(0.0,0.6)+
...
pdf("plots/Manuscript/5Bii-VMH.1, VMH.2 to VMH subclasses.pdf",height=3,width=3.75)
...
pdf("plots/Manuscript/5Ci-Sex DE correls-ARC.1, ARC.2 to ARH subclasses.pdf",height=3,width=3.75)
do.call("grid.arrange",list(arrangeGrob(grobs=arcplt,ncol=2),leg.tmp,nrow=2,heights=c(12,8)))
dev.off()
...
pdf("plots/Manuscript/5Cii-Sex DE correls-VMH.1, VMH.2 to VMH subclasses.pdf",height=3,width=3.75)
do.call("grid.arrange",list(arrangeGrob(grobs=vmhplt,ncol=2),leg.tmp,nrow=2,heights=c(12,8)))
dev.off()
```
