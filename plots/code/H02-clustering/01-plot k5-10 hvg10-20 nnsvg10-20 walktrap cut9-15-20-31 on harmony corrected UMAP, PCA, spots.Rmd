...
theme_set(theme_bw()+theme(axis.text.x = element_text(size = 10), axis.title.x = element_text(size = 10), axis.text.y = element_text(size = 11), axis.title.y = element_text(size =11), plot.title = element_text(size = 11,hjust=0.5), strip.text = element_text(size=10), legend.text = element_text(size=10), legend.title = element_text(size=10,hjust=0.5)),legend.text=element)
```
...
for (k in c(1:length(walktraps))){
    ...
    ### make base spotplots
    plt <- spatialLIBD::vis_grid_clus(hyp2.tmp,clustervar = "label",pdf_file=NULL,colors = pal,spatial = F,auto_crop = T,return_plots = T,point.size=0.5)
    ### adjust font sizes
    ...
    ### save pdf
    pdf(paste0("plots/H02-clustering/01-walktrap_cut9-15-20-31_spotplots/",names(walktraps)[k],"_cluster_spotplot.pdf"),height=9,width=13)
