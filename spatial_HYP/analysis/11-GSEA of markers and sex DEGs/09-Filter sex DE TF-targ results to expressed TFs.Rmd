...
Reload Sex DE - TFtarg GSEA results before having run collapsePathways (which could have e.g., removed a result for a TF that's relevant in the data but retained one for a TF with similar targets despite the TF NOT being expressed in the data) and the SPE (which we need for filtering on expression)
```{r}
...
```{r}
bscls <- lapply(list.files("data/06-BayesSpace/02-bayesspace60kiter_k15-20-31_out",pattern=".txt",full.names = T),fread)

names(bscls) <- gsub(list.files("data/06-BayesSpace/02-bayesspace60kiter_k15-20-31_out",pattern=".txt",full.names = T),pattern="^.*31_out/(.*)\\.txt$",replacement="\\1")
...
bscls[[4]][label %in% c("X11","X12"),label:="VMH"]
bscls[[4]][label %in% c("X3","X5"),label:="ARC"]
...
sexreslist.filt <- bpmapply(R=sexreslist,C=bscls,N=as.list(names(bscls)),FUN=function(R,C,N){
    ...
    for (i in c(1:length(R))){
        ...
        # calculate mean % spots expressing dataset wide and number of samples with expression over 0.25%. each tf occurs once per sample, so take the mean across rows with a given TF value for samplewise means, or tabulate the number of TF occurrences with the pctxpr filter to get the number of passing rows
        setnames(tffilt,"V1","pctxpr")
        tffilt[,meanxpr:=mean(pctxpr),by="tf"]
        tffilt[pctxpr>.0025,Nxpr:=.N,by="tf"]
        ...
        # FINALLY  get the TFs meeting criteria back and filter the result table to them
        
        keeptfs <- tffilt2[Nxpr>2&meanxpr>0.01,tf]
...
saveRDS(megalist.filt,"data/11-GSEA/09-Sex DE TF-target GSEA results from 04a filtered to TFs expressed in avg 1pct of cluster spots and at least .25pct of cluster spots in 3samps.RDS")
```
