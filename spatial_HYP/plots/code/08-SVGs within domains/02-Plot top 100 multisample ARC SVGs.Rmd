---
title: "02-Plot top 100 multisample ARC SVGs"
...
bscl <- bscl[cl %in% c("X4","X6"),cl:="ARC"]
bscl <- bscl[cl %in% c("X7","X12"),cl:="VMH"]

bscl <- DataFrame(bscl,row.names=bscl$rn)[colnames(hyp2),]
hyp2$bscl <- bscl$cl


## read in SVGs, filter to ≥5 nomsig samples, sort by mean rank among nomsig samples
arcsvg <- fread("processed-data/08-Nucleus-specific SVGs/02b-ARC nnSVG nomsig genes and ranks per sample.txt")
arcsvg <- arcsvg[nsigsamps>=5]
setorderv(arcsvg,"meanrank")
arcpltg <- arcsvg[1:100,.(gene_id,gene_name)]
```

### now, crank out a crapload of spotplot grids for these genes (only showing ARC for readability and paralleliz-ability).
```{r}
hyp2 <- hyp2[arcpltg$gene_id,hyp2$bscl=="ARC"]
rownames(hyp2) <- rowData(hyp2)$gene_name

mclapply(arcpltg$gene_name,mc.cores=10,FUN=function(g){
    plt <- spatialLIBD::vis_grid_gene(hyp2,geneid = g,return_plots = T,spatial=F,point_size = 1,auto_crop = T)
    ### adjust font sizes
    ...
    pdf(paste0("plots/08-SVGs within domains/02-ARCsvgs/",g,".pdf"),width=10,height=10.75)
...
loaded via a namespace (and not attached):
 ...
 [58] withr_3.0.0               doParallel_1.0.17         attempt_0.3.1            
 [61] viridis_0.6.5             DBI_1.2.3                 maps_3.4.2               
 [64] sessioninfo_1.2.2         rappdirs_0.3.3            DelayedArray_0.30.1      
 [67] rjson_0.2.21              tools_4.4.1               vipor_0.4.7              
 [70] beeswarm_0.4.0            httpuv_1.6.15             glue_1.7.0               
 [73] restfulr_0.0.15           promises_1.3.0            grid_4.4.1               
 [76] generics_0.1.3            gtable_0.3.5              tidyr_1.3.1              
 [79] ScaledMatrix_1.12.0       BiocSingular_1.20.0       utf8_1.2.4               
 [82] XVector_0.44.0            ggrepel_0.9.5             BiocVersion_3.19.1       
 [85] foreach_1.5.2             pillar_1.9.0              spam_2.10-0              
 [88] limma_3.60.3              later_1.3.2               benchmarkmeData_1.0.4    
 [91] dplyr_1.1.4               BiocFileCache_2.12.0      lattice_0.22-6           
 [94] rtracklayer_1.64.0        bit_4.0.5                 tidyselect_1.2.1         
 [97] locfit_1.5-9.10           scuttle_1.14.0            Biostrings_2.72.1        
[100] knitr_1.48                edgeR_4.2.0               xfun_0.45                
...
[121] XML_3.99-0.17             blob_1.2.4                dotCall64_1.1-1          
[124] sparseMatrixStats_1.16.0  bitops_1.0-7              viridisLite_0.4.2        
[127] scales_1.3.0              purrr_1.0.2               crayon_1.5.3             
[130] cowplot_1.1.3             KEGGREST_1.44.1          
> sessioninfo::session_info()
─ Session info ────────────────────────────────────────────────────────────────────
 setting  value
