---
title: "01-Plot top 100 multisample VMH SVGs"
...
vmhsvg <- fread("processed-data/08-Nucleus-specific SVGs/01b-VMH nnSVG nomsig genes and ranks per sample.txt")
vmhsvg <- vmhsvg[nsigsamps>=5]
setorderv(vmhsvg,"meanrank")
vmhpltg <- vmhsvg[1:100,.(gene_id,gene_name)]
```

### now, crank out a crapload of spotplot grids for these genes (only showing VMH for readability and paralleliz-ability).
```{r}
hyp2 <- hyp2[vmhsvg$gene_id,hyp2$bscl=="VMH"]
rownames(hyp2) <- rowData(hyp2)$gene_name

mclapply(vmhpltg$gene_name,mc.cores=10,FUN=function(g){
    ...
    pdf(paste0("plots/08-SVGs within domains/01-VMHsvgs/",g,".pdf"),width=10,height=10.75)
