...
```{r}
### loop through each domain and modality-disease gene set (each run is very fast with 10 cpus, so just serially run 10cpu GSEA for each cluster)
...
saveRDS(allmkdisres,"processed-data/11-GSEA/05-Common variant disease gene enrichment in markers.RDS")
```
...
other attached packages:
 [1] default_1.0.0       colorout_1.3-0.2    fgsea_1.30.0       
 [4] parallelly_1.37.1   BiocParallel_1.38.0 Biostrings_2.72.1  
 [7] GenomeInfoDb_1.40.1 XVector_0.44.0      IRanges_2.38.1     
[10] S4Vectors_0.42.1    BiocGenerics_0.50.0 data.table_1.15.4  
[13] rlang_1.1.4        
