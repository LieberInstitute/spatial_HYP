...
names(bscls)[length(bscls)] <- paste0(names(bscls)[1],"_clpsdARC_clpsdVMH")
...
### add gene symbols
```{r}
bsHna60sOut <- lapply(X=bsHna60s,FUN=function(X){
  lapply(X,FUN=function(Y){
    Z <- merge.data.table(as.data.table(Y,keep.rownames=T),tmprd[,.(gene_id,gene_name)],by.x="rn",by.y="gene_id")
    setnames(Z,"rn","gene_id")
    reordnames <- c("gene_name","logFC","P.Value","adj.P.Val","t","B","AveExpr","gene_id")
    return(Z[,..reordnames])
  })
})
...
# save results
```{r}
saveRDS(bsHna60sOut,"processed-data/09-Sex DE/01-voomLmFit_svg10-svg20-hvg20_Hmnydflt-mnn30-HmnylmbNA_BS-15-20-31-15VMHARCclpsd.RDS")
...
fwrite(rbindlist(bsHna60sOut[["BSpace_k15_HARMONYlmbna_nnsvg10"]],idcol="assay"),"processed-data/09-Sex DE/01b-voomLmFit_nnsvg10-HmnylmbNA-BS-15-singleClusts.txt",sep="\t",quote=F)
fwrite(rbindlist(bsHna60sOut[["BSpace_k15_HARMONYlmbna_hvg20_clpsdARC_clpsdVMH"]],idcol="assay"),"processed-data/09-Sex DE/01c-voomLmFit_nnsvg10-HmnylmbNA-BS-15-VMHARCclpsd.txt")
```
