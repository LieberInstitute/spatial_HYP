...
## run sex DE using voomLmFit. only on bayesspace 60kiter clusterings at k15,20,31
```{r}
...
bsHna60sOut <- lapply(X=bsHna60s,FUN=function(X){
   lapply(X,FUN=function(Y){
      Z <- merge.data.table(as.data.table(Y,keep.rownames=T),tmprd[,.(gene_id,gene_name,chromosome_name)],by.x="rn",by.y="gene_id")
...
bsHna60sOut <- lapply(X=bsHna60sOut,FUN=function(X){
...
fwrite(bsHna60sOut[["BSpace_k15_HARMONYlmbna_nnsvg10"]],"processed-data/09-Sex DE/01b-voomLmFit_nnsvg10-HmnylmbNA-BS-15-singleClusts.txt",sep="\t",quote=F)
fwrite(bsHna60sOut[["BSpace_k15_HARMONYlmbna_hvg20_clpsdARC_clpsdVMH"]],"processed-data/09-Sex DE/01c-voomLmFit_nnsvg10-HmnylmbNA-BS-15-VMHARCclpsd.txt")
```
