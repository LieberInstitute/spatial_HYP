...
### exclude V13M13-362_D1, only 207 spots.
```{r}
arc <- hyp2[,hyp2$label=="ARC"&hyp2$sample_id!="V13M13-362_D1"]
rm(hyp2)

caps <- lapply(unique(arc$sample_id),FUN=function(x){arc[,arc$sample_id==x]})
names(caps) <- unique(arc$sample_id)
rm(arc)
gc(full=T)
### iterate nnSVG over each sample, 1 sample at a time x 10 threads
### at least 1 counts in at least 5% of spots
res_list2 <- lapply(caps,FUN=function(arc.samp){
    ...
    out <- nnSVG(arc.samp,assay_name = "logcounts",n_threads = 11)
    out <- as.data.table(rowData(out),keep.rownames=T)
    message(paste0("Finished sample ",unique(arc.samp$sample_id)))
    return(out)
})
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 33) of spatial locations
# removed 17857 out of 30361 genes due to low expression
# Finished sample V12D07-075_D1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 26) of spatial locations
# removed 18914 out of 30361 genes due to low expression
# Finished sample V13M13-362_A1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 18) of spatial locations
# removed 19336 out of 30361 genes due to low expression
# Finished sample V12D07-075_A1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 70) of spatial locations
# removed 18403 out of 30361 genes due to low expression
# Finished sample V12Y31-080_A1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 50) of spatial locations
# removed 17623 out of 30361 genes due to low expression
# Finished sample V12D05-348_D1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 55) of spatial locations
# removed 17951 out of 30361 genes due to low expression
# Finished sample V12D05-350_D1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 32) of spatial locations
# removed 17404 out of 30361 genes due to low expression
# Finished sample V12D05-348_C1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 45) of spatial locations
# removed 16921 out of 30361 genes due to low expression
# Finished sample V12D05-350_C1
# Gene filtering: retaining genes with at least 1 counts in at least 5% (n = 43) of spatial locations
# removed 19335 out of 30361 genes due to low expression
```
...
saveRDS(res_list,"processed-data/08-Nucleus-specific SVGs/02a-ARC_1count-5pctspot_fulloutput.RDS")
```
...
arcsvgranks[,nasamps:=rowSums(is.na(arcsvgranks[,c(3:ncol(arcsvgranks)),with=F]))]
#
arcsvgranks[,nsigsamps:=length(caps)-nasamps]
arcsvgranks[,meanrank:=rowMeans(arcsvgranks[,c(3:(ncol(arcsvgranks)-2)),with=F],na.rm=T)]
arcsvgranks[,nasamps:=NULL]

fwrite(arcsvgranks,"processed-data/08-Nucleus-specific SVGs/02b-ARC nnSVG nomsig genes and ranks per sample.txt",sep='\t',quote=F)
```
