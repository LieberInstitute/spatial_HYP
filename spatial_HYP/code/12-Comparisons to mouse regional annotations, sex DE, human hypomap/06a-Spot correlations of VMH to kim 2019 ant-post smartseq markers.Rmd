...
## get those clusters that were illustrated with an anteroposterior pattern in fig3d. replace 'a' (first/anteriormost seqfish section) with 0 to prevent propagation of NAs.
...
fwrite(msvmh.ap.topmk,"processed-data/12-Mouse comparisons/06aII-Top5mks_per_APmapped_mspop_with1to1_hgorths.txt",sep='\t',quote=F,row.names=F)
...
#   }
  ...
  tmpd[,gene:=NULL]
  sdnames <- grep(names(tmpd),pattern="rn|-1_V",value=T)
...
# save
fwrite(spotcor.ap2,"processed-data/12-Mouse comparisons/06aIII-samplewise_meanMed_spotLogct_spear_to_APmapped_markerTstats.txt",sep='\t',quote=F,row.names=F)
```
