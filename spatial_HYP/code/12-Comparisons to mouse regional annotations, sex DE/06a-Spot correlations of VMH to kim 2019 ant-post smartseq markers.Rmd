...
## cleanup some misc
rm(dropg,drophg,dropms,multimap,multimapfish)
gc(full=T)
```
...
smrt.sce$cca_cluster_label <- gsub(smrt.sce$cca_cluster_label,pattern=" ",replacement="_")
smrt.sce$cca_cluster_label <- gsub(smrt.sce$cca_cluster_label,pattern="\\|",replacement="_")
...
saveRDS(spotcor,"processed-data/12-Mouse comparisons/06aI-VMHspot_logct_spearman_to_kim2019smartseq_markerTstats.RDS")
```
...
# so that we can subset to cases where the AP order was given (a-f, a as the most anterior), change a to 0 (because the NAs also contain a, just makes it messy)
smart_to_sfish[,APorder_fig3d:=gsub(APorder_fig3d,pattern="a",replacement="0")]
smart_to_sfish <- smart_to_sfish[APorder_fig3d %in% grep(APorder_fig3d,pattern="0|b|c|e|f",value=T)]

spotcor.ap <- spotcor[variable %in% smart_to_sfish$smartseq_clus]

## get mean and median registration per each of these clusters for ea sample. (we can add back donor info in a sec too)
spotcor.ap2 <- spotcor.ap[,xcds:=value>quantile(value,0.8),by=c("samp","variable")]
spotcor.ap2 <- spotcor.ap2[xcds==T]
setnames(spotcor.ap2,"V1","value")
spotcor.ap2 <- unique(spotcor.ap2[,mean(value,na.rm=T),by=c("samp","variable")])
spotcor.ap2 <- merge.data.table(spotcor.ap2,smart_to_sfish,by.x="variable",by.y="smartseq_clus")
