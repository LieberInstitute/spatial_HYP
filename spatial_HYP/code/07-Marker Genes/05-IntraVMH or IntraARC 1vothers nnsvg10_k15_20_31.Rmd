...
for (i in c(1:length(bscls.dt))){
    ...
    vmhres <- lapply(vmhs,FUN=function(v){
        ...
        ## stepwise set up to run registration_pairwise if two clusters, simply using reigstration_wrapper for "one-vs-all"-like otherwise)
        if(length(vmhs)==2){
           ...
           mk <- registration_wrapper(hyptmp,var_registration = "label",var_sample_id = "sample_id",gene_ensembl = "gene_id",gene_name = "gene_name")
           mkres <- as.data.table(mk[["enrichment"]],keep.rownames=T)
        }
        return(mkres)
    })
    names(vmhres) <- vmhs
