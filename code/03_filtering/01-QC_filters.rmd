...
### Ch9: QC / filtering
1. Is the tissue oriented correctly? -- REVISIT (not all anatomy/ish/slide images cross-mapped yet)
```{r}
...
# k, not non-equivalency per se
tmp[is.na(expr_chrM_ratio)]

# just zero MT counts --> NA for the auto % calculation. so we're okay as long as we use the already-calculated column.
rm(tmp,mitogenes)
# 
...
hyp3 <- addPerCellQC(hyp2, subsets = list(mito = is_mito))
